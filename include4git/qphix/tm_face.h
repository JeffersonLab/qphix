 ...
 // Accumulate received back T face (made by packTFaceForwPlus) 
						...
						int cb, int dir, int fb, int isPlus)
  {
	...
	for(int pkt = low_pkt + smtid; pkt < high_pkt; pkt+=n_threads_per_core) {
	  ...
	  if(isPlus)
		tmface_finish_dir_plus<FT,veclen,soalen,compress>(inbuf, gBase, oBase, gOffs, offs, hsprefdist, gprefdist, soprefdist, beta_T, mask, dir*2+fb );
	  else
	    tmface_finish_dir_minus<FT,veclen,soalen,compress>(inbuf, gBase, oBase, gOffs, offs, hsprefdist, gprefdist, soprefdist, beta_T, mask, dir*2+fb );
    }
  } // Function
