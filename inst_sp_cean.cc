...
string GatherFVec::serialize() const 
{
  ...
  buf << v.getName() << " = _mm512_mask_i32extgather_ps(" << v.getName() 
...
string ScatterFVec::serialize() const 
{
  ...
  buf << "_mm512_i32extscatter_ps((void*)" << a->getBase() 
      << ", " << a->getOffsets() << ", " << v.getName() 
      << ", " << downConv << ", " << scale << ", _MM_HINT_NONE);" <<endl;
...
string LoadBroadcast::serialize() const 
{
  ...
  buf << v.getName() << " = _mm512_extload_ps(" << a->serialize() 
      << ", " << upConv << ", _MM_BROADCAST_1X16, _MM_HINT_NONE);" << endl;
...
#ifndef AVX512KNL
    buf << v.getName() << " = _mm512_mask_extloadunpacklo_ps(" << v.getName() 
	<< ", " << lmask << ", " << a->serialize() << ", " << upConv 
	<< ", _MM_HINT_NONE);" << endl;
#else

    if(!a->isHalfType())  {
      buf << v.getName() << " = _mm512_mask_expandloadu_ps(" << v.getName() 
	  << ", " << lmask << ", " << a->serialize() << ");" << endl;
    }
    else {
      buf << v.getName() << " = _mm512_mask_expand_ps(" << v.getName() << ", " 
	  << lmask << ", " << "_mm512_cvtph_ps(_mm256_loadu_si256((__m256i const *)" 
	  << a->serialize() << ")));" << endl;
    }	    
...
#ifndef AVX512KNL
    buf << "_mm512_mask_extpackstorelo_ps((void*)" << a->serialize() << ", " 
...
string GatherPrefetchL1::serialize() const 
{
  ...
  buf << "_mm512_prefetch_i32gather_ps(" << a->getOffsets() << ", (void*)" 
      << a->getBase() << ", " << scale << ", " << hint << ");" <<endl;
