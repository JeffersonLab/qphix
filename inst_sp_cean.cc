...
string LoadFVec::serialize() const {
	...
	buf << v.getName() << "[:] = (" << a->serialize() << ")[0:" << VLEN << "];" <<endl;
	return buf.str();
...
string StoreFVec::serialize() const {
	...
	buf << "(" << a->serialize() << ")[0:" << VLEN << "] = " << v.getName() << "[:];" <<endl;
	return buf.str();
}
...
string GatherFVec::serialize() const {
	...
	buf << v.getName() << " = _mm512_mask_i32extgather_ps(" << v.getName() << ", " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", " << upConv << ", " << scale << ", _MM_HINT_NONE);" <<endl;
#else
	buf << v.getName() << " = _mm512_mask_i32gather_ps(" << v.getName() << ", " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", " << upConv << ", " << scale << ", _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
...
#else
	buf << "_mm512_i32scatter_ps((void*)" << a->getBase() << ", " << a->getOffsets() << ", " << v.getName() << ", " << downConv << ", " << scale << ", _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
...
string LoadBroadcast::serialize() const {
	std::ostringstream buf;
	string upConv = "_MM_UPCONV_PS_NONE";
	if(a->isHalfType()) upConv = "_MM_UPCONV_PS_FLOAT16";

#ifndef USE_OLD_INTRIN
	buf << v.getName() << " = _mm512_extload_ps(" << a->serialize() << ", " << upConv << ", _MM_BROADCAST_1X16, _MM_HINT_NONE);" << endl;
#else
	buf << v.getName() << " = _mm512_load_ps(" << a->serialize() << ", " << upConv << ", _MM_BROADCAST_1X16, _MM_HINT_NONE);" << endl;
#endif
	return buf.str();
}
...
#ifndef KNLSIM
#ifndef USE_OLD_INTRIN
		buf << v.getName() << " = _mm512_mask_extloadunpacklo_ps(" << v.getName() << ", " << lmask << ", " << a->serialize() << ", " << upConv << ", _MM_HINT_NONE);" << endl;
#else
		buf << v.getName() << " = _mm512_mask_loadunpackld(" << v.getName() << ", " << lmask << ", " << a->serialize() << ", " << upConv << ", _MM_HINT_NONE );" <<endl;
#endif
#else
		if(!a->isHalfType()) 
			buf << v.getName() << " = _mm512_mask_expandloadu_ps(" << v.getName() << ", " << lmask << ", " << a->serialize() << ");" << endl;
		else {
			buf << v.getName() << " = _mm512_mask_expand_ps(" << v.getName() << ", " << lmask << ", " << 
				"_mm512_cvtph_ps(_mm256_loadu_si256((__m256i const *)" << a->serialize() << ")));" << endl;
		}	    
#endif
		return buf.str();
	}
...
public:
	...
	string serialize() const {
		...
		string lmask= mask;
		string downConv = "_MM_DOWNCONV_PS_NONE";
		if(a->isHalfType()) downConv = "_MM_DOWNCONV_PS_FLOAT16";
		if(mask.empty()) lmask = fullMask;
#ifndef KNLSIM
#ifndef USE_OLD_INTRIN
		buf << "_mm512_mask_extpackstorelo_ps((void*)" << a->serialize() << ", " << lmask << ", " << v.getName() << ", " << downConv << ", _MM_HINT_NONE);" << endl;
#else
		buf << "_mm512_mask_packstoreld((void*)" << a->serialize() << ", " << lmask << ", " << v.getName() << ", " << downConv << ", _MM_HINT_NONE);" << endl;
#endif
#else
		if(!a->isHalfType()) 
			buf << "_mm512_mask_compressstoreu_ps(" << a->serialize() << ", " << lmask << ", " << v.getName() << ");" << endl;
		else {
			buf << "_mm256_storeu_si256((__m256i*)" << a->serialize() << ", " <<
				...
				a->serialize() << ")), " << lmask << ", " << v.getName() << "), _MM_FROUND_CUR_DIRECTION));" << endl;
		}	    
#endif
		return buf.str();
...
#ifndef USE_OLD_INTRIN
	buf << "_mm512_prefetch_i32gather_ps(" << a->getOffsets() << ", (void*)" << a->getBase() << ", " << scale << ", " << hint << ");" <<endl;
#else
	buf << "_mm512_gatherpfd(" << a->getOffsets() << ", (void*)" << a->getBase() << ", _MM_FULLUPC_NONE, "  << scale << ", " << hint << ");" <<endl;
#endif
	return buf.str();
}
...
string GatherPrefetchL2::serialize() const {
	...
	buf << "_mm512_prefetch_i32gather_ps(" << a->getOffsets() << ", (void*)" << a->getBase() << ", " << scale << ", " << hint << ");"  <<endl;
#else
	buf << "_mm512_gatherpfd(" << a->getOffsets() << ", (void*)" << a->getBase() << ", _MM_UPCONV_PS_NONE, "  << scale << ", " << hint << ");" <<endl;
#endif
	return buf.str();
...
string FnMAdd::serialize() const { 
	if(mask.empty())
		...
		return  ret.getName()+" = _mm512_mask_fnmadd_ps( " + ret.getName() + "," + mask + "," +a.getName()+" , "+b.getName()+" , "+c.getName()+" );" ;
}
...
string FMAdd::serialize() const { 
	if(mask.empty())
		...
		return  ret.getName()+" = _mm512_mask_fmadd_ps( " + ret.getName() + "," + mask + "," +a.getName()+" , "+b.getName()+" , "+c.getName()+" );" ;
}
...
public:
	...
	string serialize() const {
		...
		stream << ret.getName() << " = _mm512_mask_permute4f128_ps(" << ret.getName() << ", mk" << mk << ", " << a.getName() << ", (_MM_PERM_ENUM)" << imm << ");" ;
		return stream.str();
	}
...
void unpackFVec(InstVector& ivector, const FVec& ret, Address *a, string mask)
{
	ivector.push_back( new LoadUnpackFVec(ret, a, mask));
}

void packFVec(InstVector& ivector, const FVec& ret, Address *a, string mask)
{
...
#elif SOALEN == 8
#define TRANSPOSE transpose2x2
#elif SOALEN == 16
#define TRANSPOSE transpose1x1
#else
#error " SOALEN < 4 nor supported"
#endif

void transpose(InstVector& ivector, const FVec r[], const FVec f[])
{   
	TRANSPOSE(ivector, r, f);
}
