...
{
  ostringstream outbuf;
  outbuf << mask << " = _mm256_int2mask_pd(" << value << ");"
         << endl;
  return outbuf.str();
}
...
{
  ostringstream outbuf;
  outbuf << "__m128i " << vname
         << " = _mm_load_si128((__m128i const *)" << pname << ");"
         << endl;
  return outbuf.str();
}
...
string LoadFVec::serialize() const
{
  ...
  if (mask.empty()) {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm256_load_pd(" << a->serialize()
          << ");" << endl;
    } else {
      buf << v.getName() << " = _mm256_cvtps_pd(_mm_load_ps("
          << a->serialize() << "));" << endl;
    }
  } else {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm256_maskload_pd("
          << a->serialize() << ", _mm256_castpd_si256(" << mask
          << "));" << endl;
    } else {
      buf << v.getName() << " = _mm256_cvtps_pd(_mm_maskload_ps("
          << a->serialize() << ", _mm_castps_si128(_mm256_cvtpd_ps("
          << mask << "))));" << endl;
    }
  }
...
string StoreFVec::serialize() const
{
  ...
  if (streaming) {
    if (!a->isHalfType()) {
      buf << "_mm256_stream_pd(" << a->serialize() << ", "
          << v.getName() << ");" << endl;
    } else {
      buf << "_mm_stream_ps(" << a->serialize()
          << ", _mm256_cvtpd_ps(" << v.getName() << "));" << endl;
    }
  } else {
    if (!a->isHalfType()) {
      buf << "_mm256_store_pd(" << a->serialize() << ", "
          << v.getName() << ");" << endl;
    } else {
...
string GatherFVec::serialize() const
{
  ...
  if (mask.empty()) {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm256_i32gather_pd("
          << a->getBase() << ", " << a->getOffsets()
          << ", _MM_SCALE_8);" << endl;
    } else {
      buf << v.getName() << " = _mm256_cvtps_pd(_mm_i32gather_ps("
          << a->getBase() << ", " << a->getOffsets()
          << ", _MM_SCALE_4));" << endl;
    }
  } else {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm256_mask_i32gather_pd("
          << v.getName() << ", " << a->getBase() << ", "
          << a->getOffsets() << ", " << mask << ", _MM_SCALE_8);"
          << endl;
    } else {
      buf << v.getName()
          << " = _mm256_cvtps_pd(_mm__mask_i32gather_ps("
          << v.getName() << ", " << a->getBase() << ", "
...
string LoadBroadcast::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm256_broadcast_sd("
        << a->serialize() << ");" << endl;
  } else {
    buf << v.getName() << " = _mm256_cvtps_pd(_mm_broadcast_ss("
        << a->serialize() << "));" << endl;
  }
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm256_mul_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_mul_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
string FnMAdd::serialize() const
{
  ...
  if (mask.empty()) {
    return ret.getName() + " = _mm256_sub_pd(" + c.getName() +
           ", _mm256_mul_pd(" + a.getName() + " , " + b.getName() +
           "));";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_sub_pd(" + c.getName() + ", _mm256_mul_pd(" +
  ...
  if (mask.empty()) {
    return ret.getName() + " = _mm256_fnmadd_pd( " + a.getName() +
           " , " + b.getName() + " , " + c.getName() + " );";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_fnmadd_pd( " + a.getName() + " , " +
...
string FMAdd::serialize() const
{
  ...
  if (mask.empty()) {
    return ret.getName() + " = _mm256_add_pd(_mm256_mul_pd(" +
           a.getName() + ", " + b.getName() + "), " + c.getName() +
           " );";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_add_pd(_mm256_mul_pd(" + a.getName() + ", " +
           b.getName() + "), " + c.getName() + "), " + mask + ");";
  }
  ...
  if (mask.empty()) {
    return ret.getName() + " = _mm256_fmadd_pd( " + a.getName() +
           " , " + b.getName() + " , " + c.getName() + " );";
  } else {
    return ret.getName() + " =_mm256_blendv_pd(" + ret.getName() +
           ", _mm256_fmadd_pd( " + a.getName() + " , " +
           b.getName() + " , " + c.getName() + " ), " + mask + ");";
  }
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm256_add_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_add_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm256_sub_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm256_blendv_pd(" + ret.getName() +
           ", _mm256_sub_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " =  _mm256_insertf128_pd("
          << v.getName() << ", _mm_load_pd(" << a->serialize()
          << "), " << num << ");" << endl;
    } else {
      buf << v.getName() << " =  _mm256_insertf128_pd("
          << v.getName()
          << ", _mm_cvtps_pd(_mm_castpd_ps(_mm_load_sd((double*)"
...
class StoreHalfFVec : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << "_mm_store_pd(" << a->serialize()
          << ", _mm256_extractf128_pd(" << v.getName() << ", "
          << num << "));" << endl;
    } else {
      buf << "_mm_store_sd((double*)" << a->serialize()
          << ", _mm_castps_pd(_mm_cvtpd_ps(_mm256_extractf128_pd("
          << v.getName() << ", " << num << "))));" << endl;
    }
  ...
  string serialize() const
  {
    ...
    if (!a1->isHalfType()) {
      if (forward) {
        if (soalen == 4) {
          buf << v.getName()
              << " =  _mm256_blend_pd(_mm256_loadu_pd("
              << a1->serialize() << "), _mm256_broadcast_sd("
        ...
        if (soalen == 4) {
          buf << v.getName()
              << " =  _mm256_blend_pd(_mm256_loadu_pd(("
              << a2->serialize() << ")-1), _mm256_broadcast_sd("
              << a1->serialize() << "), 1);" << endl;
        } else {
          buf << v.getName() << " =  _mm256_insertf128_pd("
              << v.getName() << ", _mm_blend_pd(_mm_loadu_pd(("
      ...
      if (forward) {
        if (soalen == 4) {
          buf << v.getName()
              << " =  _mm256_cvtps_pd(_mm_blend_ps(_mm_loadu_ps("
          ...
          buf << v.getName() << " =  _mm256_insertf128_pd("
              << v.getName()
              << ", _mm_cvtps_pd(_mm_blend_ps(_mm_loadu_ps("
              << a1->serialize() << "), _mm_broadcast_ss("
              << a2->serialize() << "), " << (1 << (soalen - 1))
              << ")), " << soanum << ");" << endl;
        }
...
class CondInsertFVecElement : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm256_blend_pd(" << v.getName()
          << ", _mm256_broadcast_sd(" << a->serialize() << "), "
          << (1 << pos) << ");" << endl;
    } else {
      buf << v.getName() << " = _mm256_blend_pd(" << v.getName()
          << ", _mm256_cvtps_pd(_mm_broadcast_ss(" << a->serialize()
          << ")), " << (1 << pos) << ");" << endl;
    }
 ...
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      ...
      buf << "((int*)" << a->serialize()
          << ")[0] = _mm_extract_ps(_mm256_cvtpd_ps(" << v.getName()
          << "), " << pos << ");" << endl;
    }
