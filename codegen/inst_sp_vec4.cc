...
string LoadFVec::serialize() const
{
  ...
  if (mask.empty()) {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm_load_ps(" << a->serialize()
          << ");" << endl;
    } else {
...
string LoadBroadcast::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm_set1_ps(*" << a->serialize()
        << ");" << endl;
  } else {
    printf("Error: Half type not supported for SSE\n");
    exit(1);
...
{
  std::ostringstream buf;
  buf << "#error \"Gather Prefetch is not supported in SSE\""
      << endl;
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_mul_ps( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_ps(" + ret.getName() +
           ", _mm_mul_ps( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_sub_ps(" + c.getName() +
           ", _mm_mul_ps(" + a.getName() + " , " + b.getName() +
           "));";
  } else {
    return ret.getName() + " = _mm_blendv_ps(" + ret.getName() +
           ", _mm_sub_ps(" + c.getName() + ", _mm_mul_ps(" +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_add_ps(_mm_mul_ps(" +
           a.getName() + ", " + b.getName() + "), " + c.getName() +
           " );";
  } else {
    return ret.getName() + " = _mm_blendv_ps(" + ret.getName() +
           ", _mm_add_ps(_mm_mul_ps(" + a.getName() + ", " +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_add_ps( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_ps(" + ret.getName() +
           ", _mm_add_ps( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_sub_ps( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_ps(" + ret.getName() +
           ", _mm_sub_ps( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
 ...
 public:
  ...
  string serialize() const
  {
    ...
    if (!a1->isHalfType()) {
      if (forward) {
        if (soalen == 4) {
          buf << v.getName() << " =  _mm_blend_ps(_mm_loadu_ps("
              << a1->serialize() << "), _mm_set1_ps(*"
...
class CondInsertFVecElement : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm_blend_ps(" << v.getName()
          << ", _mm_set1_ps(*" << a->serialize() << "), "
          << (1 << pos) << ");" << endl;
    } else {
      printf("Error: Half type not supported for SSE\n");
      exit(1);
...
class CondExtractFVecElement : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << "((int*)" << a->serialize() << ")[0] = _mm_extract_ps("
          << v.getName() << ", " << (pos % 4) << ");" << endl;
    } else {
      printf("Error: Half type not supported for SSE\n");
      exit(1);
    }
               ...
               int soalen)
{
  ...
  default:
    printf("SOALEN = %d Not Supported at %s:%d (only SOALEN = 4 "
           "supported)\n",
           soalen,
           __FILE__,
           __LINE__);
