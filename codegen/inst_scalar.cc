...
// XXX The two following `#if` look like they are complementary to
// each other.
// Perhaps it would be good to introduce a named preprocessor
// constant for
// those, like `INITIALIZE_AT_DECLARATION`. Then one could change
// both `#if` at
// the same time.
               ...
               int soalen)
{
  ...
  default:
    printf(
        "SOALEN = %d Not Supported (only SOALEN = 1 supported)\n",
        soalen);
  }
}
