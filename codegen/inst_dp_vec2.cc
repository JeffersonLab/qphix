...
string LoadFVec::serialize() const
{
  ...
  if (mask.empty()) {
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm_load_pd(" << a->serialize()
          << ");" << endl;
    } else {
...
string LoadBroadcast::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm_loaddup_pd(" << a->serialize()
        << ");" << endl;
  } else {
    printf("ERROR: Half type not supported for SSE\n");
    exit(1);
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_mul_pd( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_pd(" + ret.getName() +
           ", _mm_mul_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_sub_pd(" + c.getName() +
           ", _mm_mul_pd(" + a.getName() + " , " + b.getName() +
           "));";
  } else {
    return ret.getName() + " = _mm_blendv_pd(" + ret.getName() +
           ", _mm_sub_pd(" + c.getName() + ", _mm_mul_pd(" +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_add_pd(_mm_mul_pd(" +
           a.getName() + ", " + b.getName() + "), " + c.getName() +
           " );";
  } else {
    return ret.getName() + " = _mm_blendv_pd(" + ret.getName() +
           ", _mm_add_pd(_mm_mul_pd(" + a.getName() + ", " +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_add_pd( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_pd(" + ret.getName() +
           ", _mm_add_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm_sub_pd( " + a.getName() + " , " +
           b.getName() + " );";
  } else {
    return ret.getName() + " = _mm_blendv_pd(" + ret.getName() +
           ", _mm_sub_pd( " + a.getName() + " , " + b.getName() +
           "), " + mask + ");";
  }
}
 ...
 public:
  ...
  string serialize() const
  {
    ...
    if (!a1->isHalfType()) {
      if (forward) {
        if (soalen == 2) {
          buf << v.getName() << " =  _mm_blend_pd(_mm_loadu_pd("
...
class CondInsertFVecElement : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm_blend_pd(" << v.getName()
          << ", _mm_loaddup_pd(" << a->serialize() << "), "
          << (1 << pos) << ");" << endl;
    } else {
      printf("ERROR: Half type not supported for SSE\n");
      exit(1);
               ...
               int soalen)
{
  ...
  default:
    printf("SOALEN = %d Not Supported at %s:%d (only SOALEN = 2 "
           "supported)\n",
           soalen,
           __FILE__,
           __LINE__);
