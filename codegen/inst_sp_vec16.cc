...
string DeclareOffsets::serialize() const
{
  ...
  outbuf << "__m512i " << vname << " = _mm512_load_epi32(" << pname
         << ");" << endl;
  return outbuf.str();
}
...
string LoadFVec::serialize() const
{
  ...
  if (upConv == "_MM_UPCONV_PS_FLOAT16") {
    buf << v.getName() << " = _mm512_mask_cvtph_ps(" << v.getName()
        << ", " << lmask << ", "
        << "_mm256_load_si256((__m256i*)" << a->serialize() << "));"
        << endl;
  } else {
    buf << v.getName() << " = _mm512_mask_load_ps(" << v.getName()
        << ", " << lmask << ", " << a->serialize() << ");" << endl;
  }
#else
  buf << v.getName() << " = _mm512_mask_extload_ps(" << v.getName()
      << ", " << lmask << ", " << a->serialize() << ", " << upConv
      << ", _MM_BROADCAST32_NONE, _MM_HINT_NONE);" << endl;
#endif
...
string StoreFVec::serialize() const
{
  ...
  if (streaming) {
#ifdef AVX512
    buf << "_mm512_stream_ps(" << a->serialize() << ","
        << v.getName() << ");" << endl;
#else
    buf << "_mm512_storenrngo_ps(" << a->serialize() << ","
        << v.getName() << ");" << endl;
#endif
  } else {
#ifdef AVX512
    if (downConv == "_MM_DOWNCONV_PS_FLOAT16") {
      // buf << "_mm256_store_si256((__m256i *)" << a->serialize()
      // << ", _mm512_cvtps_ph(" << v.getName() << ",
...
#else
#endif
      buf << "_mm512_extstore_ps(" << a->serialize() << ","
          << v.getName() << ", " << downConv << ", _MM_HINT_NONE);"
          << endl;
  }
...
string ScatterFVec::serialize() const
{
  ...
  buf << "_mm512_i32extscatter_ps(" << a->getBase() << ", "
      << a->getOffsets() << ", " << v.getName() << ", " << downConv
...
#ifdef AVX512
  if (upConv == "_MM_UPCONV_PS_FLOAT16") {
    buf << v.getName() << " = _mm512_cvtph_ps(_mm256_set1_epi16(*"
        << a->serialize() << "));" << endl;
  } else {
    buf << v.getName() << " = _mm512_set1_ps(*" << a->serialize()
        << ");" << endl;
  }
#else
  buf << v.getName() << " = _mm512_extload_ps(" << a->serialize()
      << ", " << upConv << ", _MM_BROADCAST_1X16, _MM_HINT_NONE);"
      << endl;
#endif
...
{
 public:
  PackStoreFVec(const FVec &v_, const Address *a_, string mask_)
      : v(v_), a(a_), mask(mask_)
  {
  }
  ...
  string serialize() const
  {
    ...
    buf << "_mm512_mask_extpackstorelo_ps(" << a->serialize()
        << ", " << lmask << ", " << v.getName() << ", " << downConv
        << ", _MM_HINT_NONE);" << endl;
#else
    ...
    if (!a->isHalfType()) {
      ...
      buf << "_mm256_storeu_si256((__m256i*)" << a->serialize()
          << ", "
          << "_mm512_cvtps_ph(_mm512_mask_compress_ps(_mm512_cvtph_"
             "ps(_mm256_loadu_si256((__m256i const *)"
          << a->serialize() << ")), " << lmask << ", "
          //<< v.getName() << "), _MM_FROUND_CUR_DIRECTION));" <<
          //endl;
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_mul_ps( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_mul_ps( " +
           ret.getName() + "," + mask + "," + a.getName() + " , " +
           b.getName() + " );";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_fnmadd_ps(" + a.getName() +
           ", " + b.getName() + ", " + c.getName() + ");";
  } else {
    return ret.getName() + " = _mm512_mask_mov_ps(" +
           ret.getName() + ", " + mask + ", _mm512_fnmadd_ps(" +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_fmadd_ps(" + a.getName() +
           ", " + b.getName() + ", " + c.getName() + ");";
  } else {
    return ret.getName() + " = _mm512_mask_mov_ps(" +
           ret.getName() + ", " + mask + ", _mm512_fmadd_ps(" +
           a.getName() + ", " + b.getName() + ", " + c.getName() +
           "));";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_add_ps( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_add_ps( " +
           ret.getName() + "," + mask + "," + a.getName() + " , " +
           b.getName() + " );";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_sub_ps( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_sub_ps( " +
           ret.getName() + "," + mask + "," + a.getName() + " , " +
           b.getName() + " );";
  }
}
...
{
 public:
  Shuffle32x4(const FVec &ret_,
              const string mk_,
              const FVec &a_,
              const FVec &b_,
              const int imm_)
      : ret(ret_), mk(mk_), a(a_), b(b_), imm(imm_)
  {
  }
  string serialize() const
  {
    ostringstream stream;
    stream << ret.getName() << " = _mm512_mask_shuffle_f32x4("
           << ret.getName() << ", " << mk << ", " << a.getName()
           << ", " << b.getName() << ", " << imm << ");";
    return stream.str();
  }
...
void shuffle32x4(InstVector &ivector,
                 const FVec &ret,
                 const string mk,
                 const FVec &a,
                 const FVec &b,
...
void transpose4x4(InstVector &ivector,
                  const FVec r[4],
                  const FVec f[4])
{
#ifndef AVX512
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      string localmask = mk.str();
      shuffle32x4(
          ivector, r[i], localmask, f[j], f[j + 2], 0x55 * i);
    }
  }
#endif
}
                  ...
                  const FVec f[2])
{
  ...
  for (int i = 0; i < 2; i++) {
    shuffle32x4(
        ivector, r[i], fullMask, f[0], f[1], 0x44 + 0xAA * i);
  }
#endif
}
