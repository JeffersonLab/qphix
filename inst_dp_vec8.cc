...
string DeclareOffsets::serialize() const {
	ostringstream outbuf;
#ifndef USE_OLD_INTRIN
	outbuf << "__m512i " << vname << " = _mm512_mask_load_epi32(_mm512_setzero_epi32(), " << fullMask << ", " << pname << ");" << endl;
#else
	outbuf << "__m512i " << vname << " = _mm512_mask_load_epi32(_mm512_setzero_epi32(), " << fullMask << ", " << pname << ", _MM_UPCONV_EPI32_NONE, _MM_BROADCAST32_NONE, _MM_HINT_NONE);" << endl;
#endif
	return outbuf.str();
}
...
#else
	if(!a->isHalfType())
		buf << v.getName() << " = _mm512_mask_load_pd(" << v.getName() << ", " << lmask << ", "  << a->serialize() << ", _MM_UPCONV_PD_NONE, _MM_BROADCAST64_NONE, _MM_HINT_NONE);" <<endl;
	else
		buf << v.getName() << " = _mm512_mask_cvtps_pd(" << v.getName() << ", " << lmask << ", "  << 
		"_mm512_mask_loadunpacklo_ps(_mm512_undefined_ps(), " << fullMask << ", " << a->serialize() << ", _MM_UPCONV_PS_NONE, _MM_HINT_NONE));" <<endl;
#endif
	return buf.str();
...
#else
	if(!a->isHalfType())
		buf << "_mm512_store_pd(" << a->serialize() << "," << v.getName() <<  ", " << "_MM_UPCONV_PD_NONE, _MM_HINT_NONE);" <<endl;
	else
		buf << "_mm512_mask_packstorelo_ps(" << a->serialize() << ", " << fullMask <<  ", _mm512_cvtpd_ps(" << v.getName() <<  "), " << "_MM_DOWNCONV_PS_NONE, _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
}
...
#ifndef USE_OLD_INTRIN
	if(!a->isHalfType())
		buf << v.getName() << " = _mm512_mask_i32logather_pd(" << v.getName() << ", " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", _MM_SCALE_8);" <<endl;
	else
		buf << v.getName() << " = _mm512_mask_cvtpslo_pd(" << v.getName() << ", " << lmask << ", " << "_mm512_mask_i32gather_ps(_mm512_undefined_ps(), " << ", " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", _MM_SCALE_4));" <<endl;
#else
	if(!a->isHalfType())
		buf << v.getName() << " = _mm512_mask_i32gather_pd(" << v.getName() << ", " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", _MM_UPCONV_PD_NONE, _MM_SCALE_8, _MM_HINT_NONE);" <<endl;
	else
		buf << v.getName() << " = _mm512_mask_cvtps_pd(" << v.getName() << ", " << lmask << ", "  << "_mm512_mask_i32gather_ps(_mm512_undefined_ps(), " << lmask << ", " << 
		a->getOffsets() << ", (void*)" << a->getBase()  << ", _MM_UPCONV_PS_NONE, _MM_SCALE_4, _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
...
#ifndef USE_OLD_INTRIN
	if(!a->isHalfType())
		buf << "_mm512_i32loscatter_pd((void*)" << a->getBase() << ", " << a->getOffsets() << ", " << v.getName() << ", _MM_SCALE_4);" <<endl;
	else
		buf << "_mm512_mask_i32scatter_ps((void*)" << a->getBase() << ", " << fullMask << ", " << a->getOffsets() << ", _mm512_cvtpd_pslo(" << v.getName() << "), _MM_SCALE_4);" <<endl;
#else
	if(!a->isHalfType())
		buf << "_mm512_i32scatter_pd((void*)" << a->getBase() << ", " << a->getOffsets() << ", " << v.getName() << ", _MM_DOWNCONV_PD_NONE, _MM_SCALE_8, _MM_HINT_NONE);" <<endl;
	else
		buf << "_mm512_mask_i32scatter_ps((void*)" << a->getBase() << ", " << fullMask << ", " << a->getOffsets() << ", _mm512_cvtpd_ps(" << v.getName() << "), _MM_DOWNCONV_PS_NONE, _MM_SCALE_4, _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
...
#ifndef USE_OLD_INTRIN
	if(!a->isHalfType())
		buf << v.getName() << " = _mm512_extload_pd(" << a->serialize() << ", _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);" << endl;
	else
		buf << v.getName() << " = _mm512_cvtpslo_pd(_mm512_extload_ps(" << a->serialize() << ", _MM_UPCONV_PS_NONE, _MM_BROADCAST_1X16, _MM_HINT_NONE));" << endl;
#else
	if(!a->isHalfType())
		buf << v.getName() << " = _mm512_load_pd(" << a->serialize() << ", _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);" << endl;
	else
		buf << v.getName() << " = _mm512_cvtps_pd(_mm512_load_ps(" << a->serialize() << ", _MM_UPCONV_PS_NONE, _MM_BROADCAST_1X16, _MM_HINT_NONE));" << endl;
#endif
	return buf.str();
}
...
#ifndef USE_OLD_INTRIN
		if(!a->isHalfType())
			buf << v.getName() << " = _mm512_mask_loadunpacklo_pd(" << v.getName() << ", " << lmask << ", " << a->serialize() << ");" << endl;
		else
			buf << v.getName() << " = _mm512_mask_cvtpslo_pd(" << v.getName() << ", " << lmask << ", " <<"_mm512_mask_loadunpacklo_ps(_mm512_undefined_ps(), " << lmask << ", " << a->serialize() << "));" << endl;
#else
		if(!a->isHalfType())
			buf << v.getName() << " = _mm512_mask_loadunpacklo_pd(" << v.getName() << ", " << lmask << ", " << a->serialize() << ", _MM_UPCONV_PD_NONE, _MM_HINT_NONE);" << endl;
		else
			buf << v.getName() << " = _mm512_mask_cvtps_pd(" << v.getName() << ", " << lmask << ", " <<"_mm512_mask_loadunpacklo_ps(_mm512_undefined_ps(), " << lmask << ", " << a->serialize() << ", _MM_UPCONV_PS_NONE, _MM_HINT_NONE));" << endl;
#endif
#else
		if(!a->isHalfType()) 
			buf << v.getName() << " = _mm512_mask_expandloadu_pd(" << v.getName() << ", " << lmask << ", " << a->serialize() << ");" << endl;
		else {
			buf << v.getName() << " = _mm512_mask_cvtpslo_pd(" << v.getName() << ", " << lmask << ", " <<"_mm512_mask_expandloadu_ps(_mm512_undefined_ps(), " << lmask << ", " << a->serialize() << "));" << endl;
		}	    
#endif
		return buf.str();
	}
...
#ifndef USE_OLD_INTRIN
		if(!a->isHalfType()) 
			buf << "_mm512_mask_packstorelo_pd((void*)" << a->serialize() << ", " << lmask << ", " << v.getName() << ");" << endl;
		else
			buf << "_mm512_mask_packstorelo_ps((void*)" << a->serialize() << ", " << lmask << ", _mm512_cvtpd_pslo(" << v.getName() << "));" << endl;
#else
		if(!a->isHalfType()) 
			buf << "_mm512_mask_packstorelo_pd((void*)" << a->serialize() << ", " << lmask << ", " << v.getName() << ", _MM_DOWNCONV_PD_NONE, _MM_HINT_NONE);" << endl;
		else
			buf << "_mm512_mask_packstorelo_ps((void*)" << a->serialize() << ", " << lmask << ", _mm512_cvtpd_ps(" << v.getName() << "), _MM_DOWNCONV_PS_NONE, _MM_HINT_NONE);" << endl;
#endif
#else
		if(!a->isHalfType()) 
			buf << "_mm512_mask_compressstoreu_pd(" << a->serialize() << ", " << lmask << ", " << v.getName() << ");" << endl;
		else
			buf << "_mm512_mask_compressstoreu_ps(" << a->serialize() << ", " << lmask << ", _mm512_cvtpd_pslo(" << v.getName() << "));" << endl;	    
#endif
		return buf.str();
...
PrefetchL1::PrefetchL1( const Address* a_, int type) : a(a_) {
	...
	switch (type) {
		case 0: hint = "_MM_HINT_T0"; break;
		case 1: hint = "_MM_HINT_NTA"; break;
...
PrefetchL2::PrefetchL2( const Address* a_, int type) : a(a_) {
	// Type: 0 - none, 1 - NT, 2 - Ex, 3 - NT+Ex
	switch(type) {
		case 0: hint = "_MM_HINT_T1"; break;
		case 1: hint = "_MM_HINT_T2"; break;
...
string GatherPrefetchL1::serialize() const {
	...
	if(a->isHalfType()) scale = "_MM_SCALE_4";
#ifndef USE_OLD_INTRIN
	buf << "_mm512_mask_prefetch_i32gather_ps(" << a->getOffsets() << ", " << fullMask << ", (void*)" << a->getBase() << ", " << scale << ", " << hint << ");" <<endl;
#else
	buf << "_mm512_gatherpfd(" << a->getOffsets() << ", " << fullMask << ", (void*)" << a->getBase() << ", _MM_FULLUPC_NONE, " << scale << ", " << hint << ");" <<endl;
#endif
	return buf.str();
}
...
string GatherPrefetchL2::serialize() const {
	std::ostringstream buf;
	string scale = "_MM_SCALE_4";
	if(a->isHalfType()) scale = "_MM_SCALE_2";
#ifndef USE_OLD_INTRIN
	buf << "_mm512_mask_prefetch_i32gather_ps(" << a->getOffsets() << ", " << fullMask << ", (void*)" << a->getBase() << ", " << scale << ", " << hint << ");" <<endl;
#else
	buf << "_mm512_gatherpfd(" << a->getOffsets() << ", " << fullMask << ", (void*)" << a->getBase() << ", _MM_FULLUPC_NONE, " << scale << ", " << hint << ");" <<endl;
...
public:
	...
	string serialize() const {
		...
		stream << ret.getName() << " = _mm512_castps_pd(_mm512_mask_permute4f128_ps(_mm512_castpd_ps(" << ret.getName() << "), " << mk << ", _mm512_castpd_ps(" << a.getName() << "), (_MM_PERM_ENUM)" << imm << "));" ;
		return stream.str();
	}
	int numArithmeticInst() const { return 0; }
private:
	const FVec ret; const FVec a; const string mk; const int imm;
};
