...
string LoadFVec::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm512_mask_load_pd(" << v.getName()
        << ", " << lmask << ", " << a->serialize() << ");" << endl;
  } else {
    buf << v.getName() << " = _mm512_mask_cvtpslo_pd("
        << v.getName() << ", " << lmask << ", "
        << "_mm512_mask_loadunpacklo_ps(_mm512_undefined_ps(), "
        << fullMask << ", " << a->serialize() << "));" << endl;
  }
...
string StoreFVec::serialize() const
{
  ...
  if (streaming) {
#ifdef AVX512
    buf << "_mm512_stream_pd(" << a->serialize() << ","
        << v.getName() << ");" << endl;
#else
    buf << "_mm512_storenrngo_pd(" << a->serialize() << ","
        << v.getName() << ");" << endl;
#endif
  } else {
    if (!a->isHalfType()) {
      buf << "_mm512_store_pd(" << a->serialize() << ","
          << v.getName() << ");" << endl;
    } else {
      buf << "_mm512_mask_packstorelo_ps(" << a->serialize() << ", "
          << fullMask << ", _mm512_cvtpd_pslo(" << v.getName()
          << "));" << endl;
    }
  }
...
string GatherFVec::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm512_mask_i32logather_pd("
...
string ScatterFVec::serialize() const
{
  ...
  if (!a->isHalfType()) {
    buf << "_mm512_i32loscatter_pd(" << a->getBase() << ", "
        << a->getOffsets() << ", " << v.getName()
        << ", _MM_SCALE_4);" << endl;
  } else {
    buf << "_mm512_mask_i32scatter_ps(" << a->getBase() << ", "
        << fullMask << ", " << a->getOffsets()
        << ", _mm512_cvtpd_pslo(" << v.getName()
        << "), _MM_SCALE_4);" << endl;
  }
...
#ifdef AVX512
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm512_set1_pd(*" << a->serialize()
        << ");" << endl;
  } else {
    buf << v.getName() << " = _mm512_cvtpslo_pd(_mm512_set1_ps(*"
        << a->serialize() << "));" << endl;
  }
#else
  if (!a->isHalfType()) {
    buf << v.getName() << " = _mm512_extload_pd(" << a->serialize()
        << ", _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, "
           "_MM_HINT_NONE);"
        << endl;
  } else {
    buf << v.getName() << " = _mm512_cvtpslo_pd(_mm512_extload_ps("
        << a->serialize() << ", _MM_UPCONV_PS_NONE, "
...
class LoadUnpackFVec : public MemRefInstruction
{
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm512_mask_loadunpacklo_pd("
    ...
    if (!a->isHalfType()) {
      buf << v.getName() << " = _mm512_mask_expandloadu_pd("
          << v.getName() << ", " << lmask << ", " << a->serialize()
          << ");" << endl;
    } else {
      buf << v.getName() << " = _mm512_mask_cvtpslo_pd("
          << v.getName() << ", " << lmask << ", "
          << "_mm512_mask_expandloadu_ps(_mm512_undefined_ps(), "
          << lmask << ", " << a->serialize() << "));" << endl;
    }
 ...
 public:
  ...
  string serialize() const
  {
    ...
    if (!a->isHalfType()) {
      ...
      buf << "_mm512_mask_packstorelo_ps(" << a->serialize() << ", "
          << lmask << ", _mm512_cvtpd_pslo(" << v.getName() << "));"
          << endl;
    }
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_mul_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_mul_pd( " +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_fnmadd_pd(" + a.getName() +
           ", " + b.getName() + ", " + c.getName() + ");";
  } else {
    return ret.getName() + " = _mm512_mask_mov_pd(" +
           ret.getName() + ", " + mask + ", _mm512_fnmadd_pd(" +
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_fmadd_pd(" + a.getName() +
           ", " + b.getName() + ", " + c.getName() + ");";
  } else {
    return ret.getName() + " = _mm512_mask_mov_pd(" +
           ret.getName() + ", " + mask + ", _mm512_fmadd_pd(" +
           a.getName() + ", " + b.getName() + ", " + c.getName() +
           "));";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_add_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_add_pd( " +
           ret.getName() + "," + mask + "," + a.getName() + " , " +
           b.getName() + " );";
  }
}
...
{
  if (mask.empty()) {
    return ret.getName() + " = _mm512_sub_pd( " + a.getName() +
           " , " + b.getName() + " );";
  } else {
    return ret.getName() + " = _mm512_mask_sub_pd( " +
           ret.getName() + "," + mask + "," + a.getName() + " , " +
           b.getName() + " );";
  }
}
...
class Perm64x2 : public Instruction
{
 public:
  ...
  string serialize() const
  {
    ...
    stream << ret.getName() << " = "
                               "_mm512_castps_pd(_mm512_mask_"
                               "permute4f128_ps(_mm512_castpd_ps("
           << ret.getName() << "), " << mk << ", _mm512_castpd_ps("
           << a.getName() << "), (_MM_PERM_ENUM)" << imm << "));";
...
class Shuffle64x2 : public Instruction
{
 public:
  ...
  {
    ostringstream stream;
    stream << ret.getName() << " = _mm512_mask_shuffle_f64x2("
...
#ifdef AVX512
void shuffle64x2(InstVector &ivector,
                 const FVec &ret,
                 const string mk,
                 const FVec &a,
                 const FVec &b,
               ...
               int soalen)
{
  ...
  default:
    printf("SOALEN = %d Not Supported (only SOALEN = 2, 4 & 8 are "
           "supported)\n",
           soalen);
    exit(1);
  }
