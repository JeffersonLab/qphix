...
PrefetchL1::PrefetchL1( const Address* a_, int type) : a(a_) {
	...
	switch (type) {
		...
		case 2: hint = "_MM_HINT_T0"; break;
		case 3: hint = "_MM_HINT_NTA"; break;
	}
}
...
PrefetchL2::PrefetchL2( const Address* a_, int type) : a(a_) {
	...
	switch(type) {
		...
		case 2: hint = "_MM_HINT_T1"; break;
		case 3: hint = "_MM_HINT_T2"; break;
	}
}
...
public:
  ...
  private:
    const FVec ret; const FVec a; const FVec b; int imm;
};
...
{
	int pos = 0, nBits = 0;
	for(int i = 0; i < 4; i++) if(possibleMask & (1 << i)) nBits++;
	for(int i = 0; i < 4; i++) 
		if(possibleMask & (1 << i)) {
