                                  ...
                                  InstVector &l2prefs)
{
  if (l2prefs.size() == 0) {
    ...
    // cout << "After declarations Ivector size is " << ivector_size
    // << endl;
  ...
  for (int i = 0; i < ivector.size(); i++) {
    ...
    if (inst->hasAddress()) {
      MemRefInstruction *mr =
          dynamic_cast<MemRefInstruction *>(inst);

      if (mr->hasGSAddress()) {
        const GatherAddress *ga =
            dynamic_cast<const GatherAddress *>(mr->getAddress());
  ...
  for (map<string, string>::iterator it = offslist.begin();
       it != offslist.end();
       ++it) {
    ivector.insert(ivector.begin(),
                   new DeclareOffsets(it->first, it->second));
  }
}
