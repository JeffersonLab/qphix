...
def main():
    ...
    kernels = configparser.ConfigParser(interpolation=None)
    kernels.read('kernels.ini')
    ...
    for kernel_name in kernels.sections():
        kernel = kernels[kernel_name]['spec_prefix']
        kernel_pattern = kernels[kernel_name]['kernel_prefix']
    ...
    for isa, isa_data in sorted(isas.items()):
        ...
        for kernel_name in kernels.sections():
            prefix = kernels[kernel_name]['search_prefix']
        ...
        for kernel_name in kernels.sections():
            kernel = kernels[kernel_name]['spec_prefix']
            kernel_pattern = kernels[kernel_name]['kernel_prefix']
            print('Working on kernel `{}` for ISA `{}` …'.format(kernel_name, isa))
            defines = [
