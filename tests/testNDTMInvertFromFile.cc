        ...
        for (auto f_in : {0, 1}) {
            ...
            for (auto f_out : {0, 1}) {
                ...
                for (auto s_block : {0, 1}) {
                    ...
                    auto const &block_in =
                        s_block == 0 ? A_inv[f_out][f_in][isign][block].block1
                                ...
                                for (auto c_out : {0, 1, 2}) {
                                    ...
                                    for (auto v = 0; v < V; ++v) {
                                        cplx_fmacc(
                                            ...
                                            in[f_in][block][c_in][four_s_in][0]
                                              [v],
                                            in[f_in][block][c_in][four_s_in][1]
                                              [v],
                                            out[f_out][block][c_out][four_s_out]
                                               [0][v],
                                            out[f_out][block][c_out][four_s_out]
                                               [1][v]);
                                    }
                                     ...
                                     const std::string &filename)
{
    ...
    QDPIO::cout << "Plaquette on reading: w=" << w_plaq << " link=" << link
                << endl;
                ...
                for (auto isign_idx : {0, 1}) {
                    for (auto s : {0, 1}) {
                        for (auto c : {0, 1, 2}) {
                            auto const sc = 3 * s + c;
                            for (auto v = 0; v < V; ++v) {
                                A_inv[f1][f2][isign_idx][block]
                                    .block1[sc][sc][0][v] = tau_0_factor;
                                A_inv[f1][f2][isign_idx][block]
                                    .block2[sc][sc][0][v] = tau_0_factor;
                                ...
                                if (f1 == f2) {
                                    ...
                                    A_inv[f1][f2][isign_idx][block]
                                        .block1[sc][sc][1][v] = tau_1_factor;
                                    A_inv[f1][f2][isign_idx][block]
                                        .block2[sc][sc][1][v] = tau_1_factor;
                                }
                            }
                        }
                    }
...
void testNDTMInvertFromFile::run(void)
{
    ...
    //std::string filename("/home/mu/Dokumente/Studium/Master_Science_Physik/A100.24_L24_T48_beta190_mul0100_musig150_mudel190_kappa1632550/conf.2999");
    std::string filename("/home/mu/Dokumente/Studium/Master_Science_Physik/test8x8x8x16/conf.0000");
    double mass = -0.3550;
