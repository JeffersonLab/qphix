...
void testDslashFull::run(void)
{
    ...
    QDPIO::cout << "Pad Factors: PadXY=" << PadXY << " PadXYZ=" << PadXYZ
                << endl;
    ...
    if (precision == FLOAT_PREC) {
        ...
        }
        {
            ...
            if (soalen == 1) {
                ...
                testDslashAChiMBDPsiWrapper<float, VECLEN_SP, 1, UF, PhiF>(
                    u_in);
...
void testDslashFull::testDslash(const U &u, int t_bc)
{
    ...
    u_test[3] *=
        where(Layout::latticeCoordinate(3) == (Layout::lattSize()[3] - 1),
              Real(t_boundary),
              Real(1));
    ...
    for (int isign = 1; isign >= -1; isign -= 2) {
        for (int cb = 0; cb < 2; cb++) {
            ...
            chi2 = zero;
            QDPIO::cout << "chi2_norm[cb=0]=" << norm2(chi2, rb[0])
                        << std::endl;
            QDPIO::cout << "chi2_norm[cb=1]=" << norm2(chi2, rb[1])
                        << std::endl;
            QDPIO::cout << "psi_norm[cb=0]=" << norm2(psi, rb[0]) << std::endl;
            QDPIO::cout << "psi_norm[cb=1]=" << norm2(psi, rb[1]) << std::endl;
                                    ...
                                    for (int c = 0; c < Nc; c++) {
                                        ...
                                        if (toBool(fabs(dr) >
                                            ...
                                            QDPIO::cout
                                                ...
                                                << " spin=" << s
                                                << " color=" << c << " Diff = "
                                                << diff.elem(rb[target_cb]
                                                                 .start() +
                                                             ind)
                                                       .elem(s)
                                                       .elem(c)
                                                << "  chi = "
                                                << chi.elem(rb[target_cb]
                                                                .start() +
                                                            ind)
                                                       .elem(s)
                                                       .elem(c)
                                                << " qdp++ ="
                                                << chi2.elem(rb[target_cb]
                                                                 .start() +
                                                             ind)
                                                       .elem(s)
                                                       .elem(c)
        ...
        for (int cb = 0; cb < 2; cb++) {
            ...
            if (toBool(diff_norm >= tolerance<T>::small)) {
                for (int i = 0; i < rb[target_cb].siteTable().size(); i++) {
                    for (int s = 0; s < Ns; s++) {
                        for (int c = 0; c < Nc; c++) {
                            QDPIO::cout << "site=" << i << " spin=" << s
                                        << " color=" << c << " Diff = "
                                        << diff.elem(rb[target_cb].start() + i)
                                               .elem(s)
                                               .elem(c)
                                        << endl;
                        }
        ...
        for (int cb = 0; cb < 2; ++cb) {
            ...
            if (toBool(diff_norm >= tolerance<T>::small)) {
                for (int i = 0; i < rb[cb].siteTable().size(); i++) {
                    for (int s = 0; s < Ns; s++) {
                        for (int c = 0; c < Nc; c++) {
                            Double re = Double(diff.elem(rb[cb].start() + i)
                                                   .elem(s)
                                                   .elem(c)
                                                   .real());
                            Double im = Double(diff.elem(rb[cb].start() + i)
                                                   .elem(s)
                                                   .elem(c)
                                                   .imag());
    ...
    for (int cb = 0; cb < 2; ++cb) {
...

        {
            ...
            for (int isign = 1; isign >= -1; isign -= 2) {
                ...
                Phi diff = chi2 - psi;
                Double true_norm =
                    sqrt(norm2(diff, rb[cb]) / norm2(psi, rb[cb]));
                QDPIO::cout << "BiCGStab Solve isign=" << isign
                            << " True norm is: " << true_norm << endl;
                assertion(
                    toBool(true_norm < (rsd_target + tolerance<T>::small)));
                int Nxh = Nx / 2;
                unsigned long num_cb_sites = Layout::vol() / 2;
                unsigned long total_flops =
                    (site_flops + (72 + 2 * 1320) * mv_apps) * num_cb_sites;
                masterPrintf("BICGSTAB Solve isign=%d, iters=%d MV apps=%lu "
                             ...
                             site_flops);
                masterPrintf(
                    "BICGSTAB Solve isign=%d, time=%e (sec) GFLOPS=%e\n",
                    isign,
                    (end - start),
                    1.0e-9 * (double)(total_flops) / (end - start));
            }
...
void testDslashFull::testRichardson(const U &u, int t_bc)
{
    ...
    for (int cb = 0; cb < 2; ++cb) {
        ...
        RNG::setrn(rng_seed);
        typedef typename Geometry<T1, VEC1, SOA1, compress>::SU3MatrixBlock
            GaugeOuter;
        typedef typename Geometry<T1, VEC1, SOA1, compress>::FourSpinorBlock
            SpinorOuter;

        typedef typename Geometry<T2, VEC2, SOA2, compress>::SU3MatrixBlock
            GaugeInner;
        typedef typename Geometry<T2, VEC2, SOA2, compress>::FourSpinorBlock
            SpinorInner;
        ...
        Geometry<T1, VEC1, SOA1, compress> geom_outer(
        ...
        Geometry<T2, VEC2, SOA2, compress> geom_inner(
            Layout::subgridLattSize().slice(),
            By,
            Bz,
            NCores,
        ...
        GaugeInner *packed_gauge_cb0_inner =
            (GaugeInner *)geom_inner.allocCBGauge();
        GaugeInner *packed_gauge_cb1_inner =
            (GaugeInner *)geom_inner.allocCBGauge();
        ...
        EvenOddWilsonOperator<T2, VEC2, SOA2, compress> M_inner(Mass,
                                                                u_packed_inner,
                                                                &geom_inner,
                                                                t_boundary,
                                                                aniso_fac_s,
...

        {
            ...
            InvBiCGStab<T2, VEC2, SOA2, compress> inner_solver(M_inner,
                                                               max_iters);
            InvRichardsonMultiPrec<T1,
                                   VEC1,
                                   SOA1,
                                   compress,
                                   T2,
            ...
            for (int isign = 1; isign >= -1; isign -= 2) {
                ...
                QDPIO::cout << "RICHARDSON Solve isign=" << isign
                            << " True norm is: " << true_norm << endl;
                assertion(
                    toBool(true_norm < (rsd_target + tolerance<T1>::small)));
                unsigned long num_cb_sites = Layout::vol() / 2;
                unsigned long total_flops =
                    (site_flops + (72 + 2 * 1320) * mv_apps) * num_cb_sites;

                masterPrintf(
                    "RICHARDSON Solve isign=%d time=%e (sec) GFLOPS=%e\n",
                    isign,
                    (end - start),
                    1.0e-9 * (double)(total_flops) / (end - start));
            }
