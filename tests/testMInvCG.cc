...
void MInvCGTester::run(void)
{
    ...
    defined(QPHIX_MIC_SOURCE) || defined(QPHIX_AVX512_SOURCE)
            ...
            if (soalen == 16) {
#if defined(QPHIX_MIC_SOURCE) || defined(QPHIX_AVX512_SOURCE)
                QDPIO::cout << "VECLEN = " << VECLEN_SP << " SOALEN=16 "
                            << endl;
    ...
    if (precision == HALF_PREC) {
        ...
        }
        {
            ...
            if (soalen == 16) {
                QDPIO::cout << "VECLEN = " << VECLEN_HP << " SOALEN=16 "
                            << endl;
                testMInvCGWrapper<half, VECLEN_HP, 16, UF, PhiF>(u_in);
            }
        }
#else
        QDPIO::cout << " Half Prec is only supported on MIC Targets just now "
                    << endl;
#endif
    }
#endif // If HALF-PREC
