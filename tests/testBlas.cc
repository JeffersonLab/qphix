...
void copy(  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* x1,
	    Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* x2, 
	    int N_blocks) 
{
...
#pragma omp parallel for
 for(int block=0; block < N_blocks; block++) {
	...
	for(int site=0; site < SOALEN; site++){ 
...
void resetSpinors( Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*  x1,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* x2,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* y1,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* y2,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* z1,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* z2,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* t1,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* t2,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* w1,
		   Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* w2,
		   const Geometry<float,VECLEN,SOALEN,true>& geom)
{
  double start, end;
  int Nt=geom.Nt();
  int Nz=geom.Nz();
...
testBlas::run(const int lattSize[], const int qmp_geom[]) 
{
  ...
  int Bz = Nz;
  int N_simt = Sy*Sz;
  Geometry<float, VECLEN, SOALEN,true> geom(subLattSize,
				       Ny,Nz,
				       NCores,
				       Sy,Sz,
				       PadXY,PadXYZ,1);

  typedef Geometry<float,VECLEN, SOALEN,true>::FourSpinorBlock Spinor;
  ...
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* x1=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* x2=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* y1=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* y2=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* z1=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* z2=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* t1=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* t2=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* w1=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();
  Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock* w2=(Geometry<float,VECLEN,SOALEN,true>::FourSpinorBlock*)geom.allocCBFourSpinor();

  int N_blocks =  geom.getPxyz()*geom.Nt();
  int len = N_blocks*SOALEN*4*3*2;
  float ar=0.6;
...
#else
      copySpinor<float,VECLEN,SOALEN,true>(y2, x1, geom, bt);
#endif
 ...
 //   ====================   AYPX ========================
  {
    ...
    for( int bt=1; bt <=N_simt; bt++) { 
      ...
      float *xf2 = (float *)x2;
      float *yf2 = (float *)y2;
      aypx<float,VECLEN,SOALEN,true>(ar, x2, y2, geom, bt);
      try { 
#pragma omp parallel for collapse(6)
	for(int t=0; t < Nt; t++) { 
	  for(int z=0; z < Nz; z++) { 
 ...
 //  ============ XMYNORM2SPINOR =========================
  {
    ...
    for(int bt=1; bt <=N_simt; bt++) {
      ...
      copy(t1, y2, N_blocks);
      norm2=0;
      xmyNorm2Spinor<float,VECLEN,SOALEN,true>(w2,x2,y2, norm2, geom, bt);
...
#pragma omp parallel for collapse(6)
    ...
    for( int bt =1 ; bt <= N_simt; bt++) { 
      ...
      for(int i=0; i < titers; i++) { 
	xmyNorm2Spinor<float,VECLEN,SOALEN,true>(w2, 
					    x2, 
					    y2, 
					    norm, geom, bt );
      }
...
// RMAMMPNORM2RXPAP
  {
    ...
    for(int bt=1; bt <= N_simt; bt++) { 
      ...
      float *mmp2f = (float *)mmp1f; // Unchanged
      norm2 = 0;
      rmammpNorm2rxpap<float,VECLEN,SOALEN,true>(z2, ar, t1, norm2, x2, y1, geom, bt);
...
#pragma omp parallel for collapse(6)
  ...
  // NORM2SPINOR
  {
    ...
    for(int bt =1 ; bt <=N_simt ; bt++) { 
      ...
      norm2Spinor<float,VECLEN,SOALEN,true>(norm2,y2, geom, bt);
      try { 
	assertion( toBool( fabs(norm2 - norm)/ norm < 2.0e-12 ));
	masterPrintf("Sum OK\n");
      }
    ...
    for(int bt=1; bt <= N_simt; bt++) { 
      ...
      for(int i=0; i < titers; i++) { 
	norm2Spinor<float,VECLEN,SOALEN,true>(n2res,y2, geom, bt);
      }
      double tstop = omp_get_wtime();
      double ttime=tstop-tstart;
      double BW = (1.0*len*sizeof(float)*(double)titers)/ttime;
