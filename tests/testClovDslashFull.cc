...
// Apply Optimized Dslash
            ...
            Spinor *chi_targ =
                (Spinor *)(chi.getFjit()) + target_cb * cbsize_in_blocks;
            Spinor *psi_src =
                (Spinor *)(psi.getFjit()) + source_cb * cbsize_in_blocks;
    ...
    ClovDslash<FT, V, S, compress> D32_ap(
        &geom, t_boundary, aniso_fac_s, aniso_fac_t);
    ...
    QDPIO::cout
        << "Creating Clover term using Gauge field with antiperiodic BCs"
        << endl;
    ...
    for (int cb = 0; cb < 2; cb++) {
        qdp_pack_clover<>(
            invclov_qdp_ap, invclov_packed[cb], D32_ap.getGeometry(), cb);
    }

    for (int cb = 0; cb < 2; cb++) {
        qdp_pack_clover<>(
            clov_qdp_ap, clov_packed[cb], D32_ap.getGeometry(), cb);
    }
        ...
        for (int isign = 1; isign >= -1; isign -= 2) {
            ...
            if (toBool(diff_norm > tolerance<FT>::small)) {
                for (int i = 0; i < rb[cb].siteTable().size(); i++) {
                    for (int s = 0; s < Ns; s++) {
                        for (int c = 0; c < Nc; c++) {
                            REAL re = diff.elem(rb[cb].start() + i)
                                          .elem(s)
                                          .elem(c)
                                          .real();
                            REAL im = diff.elem(rb[cb].start() + i)
                                          .elem(s)
                                          .elem(c)
                                          .imag();
