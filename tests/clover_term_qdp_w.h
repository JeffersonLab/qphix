...
template <typename U>
inline void
makeClovSiteLoop(int lo, int hi, int myId, QDPCloverMakeClovArg<U> *a)
{
    ...
    for (int ssite = lo; ssite < hi; ++ssite) {
        ...
        for (int block = 0; block < 2; block++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                tr_log_diag.elem(site).elem().elem().elem() +=
                    log(fabs(inv_d[i].elem()));
...
void QDPCloverTermT<T, U>::ldagdlinv(LatticeREAL &tr_log_diag, int cb)
{
    ...
    dispatch_to_threads(
        rb[cb].numSiteTable(), a, QDPCloverEnv::LDagDLInvSiteLoop<U>);
 ...
 */
template <typename T, typename U>
void QDPCloverTermT<T, U>::applySite(T &chi,
                                     const T &psi,
                                     int isign,
                                     int site) const
{
    ...
    if (Ns != 4) {
        QDPIO::cerr << __func__ << ": CloverTerm::applySite requires Ns==4"
                    << endl;
        QDP_abort(1);
    }
    ...
    cchi[10] = tri[site].diag[1][4] * ppsi[10] +
               tri[site].offd[1][6] * ppsi[6] + tri[site].offd[1][7] * ppsi[7] +
               tri[site].offd[1][8] * ppsi[8] + tri[site].offd[1][9] * ppsi[9] +
               conj(tri[site].offd[1][14]) * ppsi[11];

    cchi[11] =
        tri[site].diag[1][5] * ppsi[11] + tri[site].offd[1][10] * ppsi[6] +
