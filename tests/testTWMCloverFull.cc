...
// This tests dslash with anti-periodic BCs
  ...
  QDPIO::cout << "Creating QPD++ Clover term using Gauge field with antiperiodic BCs"
              << std::endl;
  ...
  for (int cb = 0; cb < 2; cb++) {
    for (int pm = 0; pm < 2; pm++) {
      qdp_pack_full_clover<>(
          invclov_qdp_ap, invclov_packed[cb][pm], D32_ap.getGeometry(), cb);
    }
  }
  QDPIO::cout << "Done." << std::endl;
...
// This tests the full even/odd operators
  ...
  for (int isign = 1; isign >= -1; isign -= 2) {
    ...
    diff[rb[1]] = chi2 - chi;
    Double diff_norm =
        sqrt(norm2(diff, rb[1])) / (Real(4 * 3 * 2 * Layout::vol()) / Real(2));
        ...
        for (int s = 0; s < Ns; s++) {
          for (int c = 0; c < Nc; c++) {
            ...
            if (toBool(fabs(re) > tolerance<FT>::small) ||
              ...
              QDPIO::cout << "site=" << i << " spin=" << s << " color=" << c
                          << " Diff = "
                          << diff.elem(rb[1].start() + i).elem(s).elem(c) << endl;
            }
          }
        }
      }
...
// This tests the CG
#if 1
  {
    ...
    expect_near(chi3, psi, 1e-9, geom, 1, "TM Clover CG");
...
// This tests the BiCG
#if 1
  {
    ...
    expect_near(chi2, psi, 1e-9, geom, 1, "TM Clover BiCGStab");
