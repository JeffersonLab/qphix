...
void testTWMCloverFull::runTest(void)
{
    ...
    QDPIO::cout << "Dslash will run with " << omp_get_max_threads()
                << " threads" << endl;
    ...
    QDPIO::cout << "Multiplying aniso coeffs into gauge field for testing"
                << endl;
    multi1d<U> u_test(4);
...
// This tests dslash
    ...
    QDPIO::cout << std::endl
                << "TESTING TWISTED-MASS CLOVER DSLASH:" << std::endl;
...
// This tests dslash with anti-periodic BCs
    ...
    t_boundary = (double)(-1);
    TMClovDslash<FT, V, S, compress> D32_ap(
        &geom, t_boundary, aniso_fac_s, aniso_fac_t);
    ...
    QDPIO::cout
        << "Creating QPD++ Clover term using Gauge field with antiperiodic BCs"
        << std::endl;
    ...
    for (int cb = 0; cb < 2; cb++) {
        for (int pm = 0; pm < 2; pm++) {
            qdp_pack_full_clover<>(
                clov_qdp_ap, clov_packed[cb][pm], D32_ap.getGeometry(), cb);
        }
    }
    QDPIO::cout << "...and QPhiX Clover Term Inverse..." << std::endl;
    for (int cb = 0; cb < 2; cb++) {
        for (int pm = 0; pm < 2; pm++) {
            qdp_pack_full_clover<>(invclov_qdp_ap,
                                   invclov_packed[cb][pm],
                                   D32_ap.getGeometry(),
                                   cb);
        }
    }
...
// This tests the full even/odd operators
    ...
    for (auto isign : {1, -1}) {
        ...
        Double diff_norm = sqrt(norm2(diff, rb[1])) /
                           (Real(4 * 3 * 2 * Layout::vol()) / Real(2));
        QDPIO::cout << "\t isign = " << isign << "  diff_norm = " << diff_norm
                    << endl;
        ...
        if (toBool(diff_norm > tolerance<FT>::small)) {
            for (int i = 0; i < rb[1].siteTable().size(); i++) {
                for (int s = 0; s < Ns; s++) {
                    for (int c = 0; c < Nc; c++) {
                        REAL re =
                            diff.elem(rb[1].start() + i).elem(s).elem(c).real();
                        REAL im =
                            diff.elem(rb[1].start() + i).elem(s).elem(c).imag();
                        if (toBool(fabs(re) > tolerance<FT>::small) ||
                            toBool(fabs(im) > tolerance<FT>::small)) {
                            QDPIO::cout
                                << "site=" << i << " spin=" << s
                                << " color=" << c << " Diff = "
                                << diff.elem(rb[1].start() + i).elem(s).elem(c)
                                << endl;
                        }
                    }
                }
