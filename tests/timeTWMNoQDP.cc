...
void
timeTWMDslashNoQDP::runTest(const int lattSize[], const int qmp_geom[])
{
...
#pragma omp parallel for collapse(4)    
  ...
  for(int isign=1; isign >= -1; isign -=2) {
	...
	double Gflops = (1320.0f+72.0f)*(double)(iters)*(double)(X1h*Ny*Nz*Nt)/1.0e9;
...
void
timeTWMDslashNoQDP::run(const int lattSize[], const int qmp_geom[])
{
