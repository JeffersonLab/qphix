...
void timeClovNoQDP::runTest(const int lattSize[], const int qmp_geom[])
{
                        ...
                        for (int c = 0; c < (compress ? 2 : 3); c++) {
                            for (int c2 = 0; c2 < 3; c2++) {
                                for (int x = 0; x < S; x++) {
                                    ...
                                    u_packed[0][block][mu][c][c2][IM][xx] =
                                        rep<FT, double>(d2);
                                    u_packed[1][block][mu][c][c2][IM][xx] =
                                        rep<FT, double>(d4);
                                }
                            }
                        } // row
            ...
            for (int z = 0; z < lZ; z++) {
                for (int y = 0; y < lY; y++) {
                    for (int s = 0; s < nvecs; s++) {
                        ...
                        int block =
                            (t * Pxyz + z * Pxy) / nyg + (y / nyg) * nvecs + s;
                        ...
                        for (int mu = 0; mu < 8; mu++) {
                            for (int row = 0; row < 2; row++) {
                                ...
                                for (int col = 0; col < 3; col++) {
                                    for (int x = 0; x < S; x++) {
                                        ...
                                        double u0_re = rep<double, FT>(
                                            u_packed[0][block][mu][row][col][RE]
                                                    [xx]);
                                        double u1_re = rep<double, FT>(
                                            u_packed[1][block][mu][row][col][RE]
                                                    [xx]);

                                        double u0_im = rep<double, FT>(
                                            u_packed[0][block][mu][row][col][IM]
                                                    [xx]);
                                        double u1_im = rep<double, FT>(
                                            u_packed[1][block][mu][row][col][IM]
                                                    [xx]);

                                        norm_row_cb0[xx] +=
                                            (u0_re * u0_re) + (u0_im * u0_im);

                                        norm_row_cb1[xx] +=
                                            (u1_re * u1_re) + (u1_im * u1_im);
                                ...
                                for (int col = 0; col < 3; col++) {
                                    for (int x = 0; x < S; x++) {
                                        ...
                                        double u0_re =
                                            rep<double, FT>(
                                                u_packed[0][block][mu][row][col]
                                                        [RE][xx]) /
                                            norm_row_cb0[xx];
                                        double u1_re =
                                            rep<double, FT>(
                                                u_packed[1][block][mu][row][col]
                                                        [RE][xx]) /
                                            norm_row_cb1[xx];
                                        ...
                                        u_packed[0][block][mu][row][col][RE]
                                                [xx] = rep<FT, double>(u0_re);
                                        u_packed[0][block][mu][row][col][IM]
                                                [xx] = rep<FT, double>(u0_im);

                                        u_packed[1][block][mu][row][col][RE]
                                                [xx] = rep<FT, double>(u1_re);
                                        u_packed[1][block][mu][row][col][IM]
                                                [xx] = rep<FT, double>(u1_im);
                                    } // x
                                } // col
                            } // row

                            {
                                for (int x = 0; x < S; x++) {
                                    ...
                                    int xx = (y % nyg) * S + x;
                                    double ar = rep<double, FT>(
                                        u_packed[0][block][mu][0][0][RE][xx]);
                                    double ai = rep<double, FT>(
                                        u_packed[0][block][mu][0][0][IM][xx]);

                                    double br = rep<double, FT>(
                                        u_packed[0][block][mu][0][1][RE][xx]);
                                    double bi = rep<double, FT>(
                                        u_packed[0][block][mu][0][1][IM][xx]);

                                    double cr = rep<double, FT>(
                                        u_packed[0][block][mu][0][2][RE][xx]);
                                    double ci = rep<double, FT>(
                                        u_packed[0][block][mu][0][2][IM][xx]);

                                    double dr = rep<double, FT>(
                                        u_packed[0][block][mu][1][0][RE][xx]);
                                    double di = rep<double, FT>(
                                        u_packed[0][block][mu][1][0][IM][xx]);

                                    double er = rep<double, FT>(
                                        u_packed[0][block][mu][1][1][RE][xx]);
                                    double ei = rep<double, FT>(
                                        u_packed[0][block][mu][1][1][IM][xx]);

                                    double fr = rep<double, FT>(
                                        u_packed[0][block][mu][1][2][RE][xx]);
                                    double fi = rep<double, FT>(
                                        u_packed[0][block][mu][1][2][IM][xx]);

                                    u_packed[0][block][mu][2][0][RE][xx] =
                                        rep<FT, double>(br * fr - bi * fi -
                                                        er * cr + ei * ci);
                                    u_packed[0][block][mu][2][0][IM][xx] =
                                        rep<FT, double>(er * ci + ei * cr -
                                                        br * fi - bi * fr);
                                    u_packed[0][block][mu][2][1][RE][xx] =
                                        rep<FT, double>(dr * cr - di * ci -
                                                        ar * fr + ai * fi);
                                    u_packed[0][block][mu][2][1][IM][xx] =
                                        rep<FT, double>(ar * fi + ai * fr -
                                                        dr * ci - di * cr);
                                    u_packed[0][block][mu][2][2][RE][xx] =
                                        rep<FT, double>(ar * er - ai * ei -
                                                        dr * br + di * bi);
                                    u_packed[0][block][mu][2][2][IM][xx] =
                                        rep<FT, double>(dr * bi + di * br -
                                                        ar * ei - ai * er);
                                }
                            }

                            {
                                for (int x = 0; x < S; x++) {
                                    ...
                                    // 3rd row reconstruction.
                                    double ar = rep<double, FT>(
                                        u_packed[1][block][mu][0][0][RE][xx]);
                                    double ai = rep<double, FT>(
                                        u_packed[1][block][mu][0][0][IM][xx]);

                                    double br = rep<double, FT>(
                                        u_packed[1][block][mu][0][1][RE][xx]);
                                    double bi = rep<double, FT>(
                                        u_packed[1][block][mu][0][1][IM][xx]);

                                    double cr = rep<double, FT>(
                                        u_packed[1][block][mu][0][2][RE][xx]);
                                    double ci = rep<double, FT>(
                                        u_packed[1][block][mu][0][2][IM][xx]);

                                    double dr = rep<double, FT>(
                                        u_packed[1][block][mu][1][0][RE][xx]);
                                    double di = rep<double, FT>(
                                        u_packed[1][block][mu][1][0][IM][xx]);

                                    double er = rep<double, FT>(
                                        u_packed[1][block][mu][1][1][RE][xx]);
                                    double ei = rep<double, FT>(
                                        u_packed[1][block][mu][1][1][IM][xx]);

                                    double fr = rep<double, FT>(
                                        u_packed[1][block][mu][1][2][RE][xx]);
                                    double fi = rep<double, FT>(
                                        u_packed[1][block][mu][1][2][IM][xx]);
                    ...
                    for (int spin = 0; spin < 4; spin++) {
                        for (int col = 0; col < 3; col++) {
                            for (int x = 0; x < S; x++) {
                                ...
                                psi_s[0][ind][col][spin][0][x] =
                                    rep<FT, double>(d1);
                                psi_s[0][ind][col][spin][1][x] =
                                    rep<FT, double>(d2);
                                psi_s[1][ind][col][spin][0][x] =
                                    rep<FT, double>(d3);
                                psi_s[1][ind][col][spin][1][x] =
                                    rep<FT, double>(d4);
                            }
                        }
                    }
                        ...
                        for (int col = 0; col < 3; col++) {
                            for (int x = 0; x < S; x++) {
                                ...
                                int ind = t * Pxyz + z * Pxy + y * nvecs +
                                          s; //((t*Nz+z)*Ny+y)*nvecs+s;
                                double d = 0;
                                chi_s[0][ind][col][spin][0][x] =
                                    rep<FT, double>(d);
                                chi_s[0][ind][col][spin][1][x] =
                                    rep<FT, double>(d);
                                chi_s[1][ind][col][spin][0][x] =
                                    rep<FT, double>(d);
                                chi_s[1][ind][col][spin][1][x] =
                                    rep<FT, double>(d);
                            }
                        }
                    }
            ...
            for (int y = 0; y < lY; y++) {
                for (int s = 0; s < nvecs; s++) {
                    for (int x = 0; x < S; x++) {
                        ...
                        int xx = (y % nyg) * S + x;
                        for (int i = 0; i < 6; i++) {
                            A_cb0[block].diag1[i][xx] =
                                rep<FT, double>((double)4.1);
                            A_inv_cb1[block].diag1[i][xx] =
                                rep<FT, double>((double)1 / (double)4.1);
                            A_cb0[block].diag2[i][xx] =
                                rep<FT, double>((double)4.1);
                            A_inv_cb1[block].diag2[i][xx] =
                                rep<FT, double>((double)1 / (double)4.1);
                        }

                        for (int i = 0; i < 15; i++) {
                            for (int reim = 0; reim < 2; reim++) {
                                A_cb0[block].off_diag1[i][reim][xx] =
                                    rep<FT, double>(0);
                                A_inv_cb1[block].off_diag1[i][reim][xx] =
                                    rep<FT, double>(0);

                                A_cb0[block].off_diag2[i][reim][xx] =
                                    rep<FT, double>(0);
                                A_inv_cb1[block].off_diag2[i][reim][xx] =
                                    rep<FT, double>(0);
                                ;
                            }
                        }
        ...
        for (int isign = 1; isign >= -1; isign -= 2) {
            for (int cb = 0; cb < 2; cb++) {
                ...
                for (int repeat = 0; repeat < 3; repeat++) {
                    ...
                    masterPrintf(
                        "\t %d iterations in %e seconds\n", iters, time);
                    masterPrintf("\t %e usec/iteration\n",
                                 1.0e6 * time / (double)iters);
                    double Gflops = 1824.0f * (double)(iters) *
    ...
    if (do_m) {
        ...
        for (int isign = 1; isign >= -1; isign -= 2) {
            ...
            for (int repeat = 0; repeat < 3; repeat++) {
                ...
                double Gflops = flops_per_iter * (double)(iters) *
                                (double)(X1h * Ny * Nz * Nt) / 1.0e9;
                double perf = Gflops / time;
                masterPrintf("\t Performance: %g GFLOPS total\n", perf);
                masterPrintf("\t              %g GFLOPS / node\n",
...
#pragma omp parallel for collapse(4)
            ...
            unsigned long total_flops =
                (site_flops + (3696) * mv_apps) * num_cb_sites;
...
void timeClovNoQDP::run(const int lattSize[], const int qmp_geom[])
{

    if (precision == FLOAT_PREC) {
        if (QPHIX_SOALEN > VECLEN_SP) {
            masterPrintf(
                "SOALEN=%d is greater than the single prec VECLEN=%d\n",
                QPHIX_SOALEN,
                VECLEN_SP);
            abort();
        }
