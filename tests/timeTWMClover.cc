...
void timeTWMClover::runTest(const int lattSize[], const int qmp_geom[])
{
    ...
    typedef
        typename Geometry<FT, V, S, compress>::FullCloverBlock FullCloverBlock;
...
#pragma omp parallel for collapse(4)
    ...
    FullCloverBlock *A_inv_cb1_minus =
        (FullCloverBlock *)geom.allocCBFullClov();
                    ...
                    for (int x = 0; x < S; x++) {
                        ...
                        for (int pm = 0; pm < 2; ++pm) {
                            for (int i = 0; i < 6; ++i) {
                                ...
                                A_cb0[pm][block].block1[i][i][0][xx] =
                                    rep<FT, double>((double)4.1);
                                A_cb0[pm][block].block1[i][i][1][xx] =
                                    rep<FT, double>((double)0);
                                A_cb0[pm][block].block2[i][i][0][xx] =
                                    rep<FT, double>((double)4.1);
                                A_cb0[pm][block].block2[i][i][1][xx] =
                                    rep<FT, double>((double)0);
