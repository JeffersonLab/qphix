...
#pragma omp parallel for collapse(4)
      ...
      for(int y = 0; y < lY; y++) {
        for(int s = 0; s < nvecs; s++) {
          for(int x = 0; x < S; x++) {
            ...
            // Zero out both clover terms & inverses for both blocks respectively
            for(int pm=0; pm<2; ++pm) {
              for(int i=0; i<6; ++i) {
                for(int j=0; j<6; ++j) {
                  for(int reim=0; reim<2; ++reim) {

                    A_cb0[pm][block].block1[i][j][reim][xx] = rep<FT,double> ( (double)0 );
                    A_cb0[pm][block].block2[i][j][reim][xx] = rep<FT,double> ( (double)0 );

                    A_inv_cb1[pm][block].block1[i][j][reim][xx] = rep<FT,double> ( (double)0 );
                    A_inv_cb1[pm][block].block2[i][j][reim][xx] = rep<FT,double> ( (double)0 );
            ...
            for(int pm=0; pm<2; ++pm) {
              for(int i=0; i<6; ++i) {
                ...
                A_inv_cb1[pm][block].block1[i][i][0][xx] = rep<FT,double> ( (double)1/double(4.1) );
                A_inv_cb1[pm][block].block1[i][i][1][xx] = rep<FT,double> ( (double)0 );
                A_inv_cb1[pm][block].block2[i][i][0][xx] = rep<FT,double> ( (double)1/double(4.1) );
                A_inv_cb1[pm][block].block2[i][i][1][xx] = rep<FT,double> ( (double)0 );
  ...
  EvenOddTMCloverOperator<FT,V,S,compress> M(u_packed, A_cb0, A_inv_cb1, &geom, t_boundary, coeff_s, coeff_t);
