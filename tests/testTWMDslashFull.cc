...
#else
        QDPIO::cout
            << " Half Prec is only supported on MIC and AVX2 Targets just now "
            << endl;
#endif
    } // HALF_PREC
                    ...
                    for (int s = 0; s < Ns; s++) {
                        for (int c = 0; c < Nc; c++) {
                            if (num_of_records < 16) {
                                num_of_records += 1;
                                Double re =
                                    Double(diff.elem(rb[target_cb].start() + i)
                                               .elem(s)
                                               .elem(c)
                                               .real());
                                Double im =
...
void testTWMDslashFull::testTWMRichardson(const U &u, int t_bc)
{
    RNG::setrn(rng_seed);
    typedef
        typename Geometry<T1, VEC1, SOA1, compress>::SU3MatrixBlock GaugeOuter;
    typedef typename Geometry<T1, VEC1, SOA1, compress>::FourSpinorBlock
        SpinorOuter;

    typedef
        typename Geometry<T2, VEC2, SOA2, compress>::SU3MatrixBlock GaugeInner;
                ...
                for (int x = 0; x < Nxh; x++) {
                    ...
                    for (int spin = 0; spin < Ns; ++spin) {
                        for (int color = 0; color < Nc; ++color) {
                            ...
                            psi.elem(rb[target_cb].start() + index)
                                .elem(spin)
                                .elem(color)
                                .real() = Alpha * y_re;
                            psi.elem(rb[target_cb].start() + index)
