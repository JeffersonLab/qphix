...
void InvBiCGStabSolve(const Real &massFactor,
                      const Real &betaFactor,
                      multi1d<U> u,
                      const T &chi,
                      T &psi,
...

{
    ...
    for (int k = 1; k <= MaxBiCGStab && !convP; k++) {
        ...
        CR beta_r = beta;
#ifdef DEBUG
        QDPIO::cout << "k=" << k << " beta=" << beta << "  omega=" << omega
                    << endl;
#endif

#ifdef DEBUG
        QDPIO::cout << "k=" << k
                    << " p_norm before p_update = " << norm2(p, rb[targetCB])
                    << endl;
#endif
...
#ifdef DEBUG
        QDPIO::cout << "k=" << k
                    << " p_norm after p_update =" << norm2(p, rb[targetCB])
                    << endl;
#endif
...
// put <r_0 | v > into tmp
        ...
        QDPIO::cout << "k=" << k << "r0_norm=" << norm2(r0, rb[targetCB])
                    << "  vnorm=" << norm2(v, rb[targetCB]) << endl;
#endif
...
#ifdef DEBUG
        QDPIO::cout << "k=" << k
                    << " r_norm before tmult=" << norm2(r, rb[targetCB])
                    << endl;
#endif
        // t = As  = Ar
        //    A(t,r,isign);
...
// then add in the alpha p
#ifdef DEBUG
        QDPIO::cout << "k=" << k << " alpha=" << alpha << " omega=" << omega
                    << endl;
#endif
        ...
        // flopcount.addSiteFlops(16*Nc*Ns,s);    // p = r + beta p - beta_omega
        // v
