...
void add_twisted_mass_to_full_clover(
    ...
    typename Geometry<FT, V, soalen, compress>::FullCloverBlock *clov_packed[2]
                                                                            [2])
{
    typedef
        typename Geometry<FT, V, soalen, compress>::FullCloverBlock FullClover;
                            ...
                            for (int c = 0; c < 3; c++) {
                                for (int spin = 0; spin < 2; spin++) {
                                    ...
                                    for (int64_t x = 0; x < soalen; x++) {
                                        ...
                                        clov_block1[cs][cs][IM][xx] +=
                                            -twisted_mass_mu;
                                        clov_block2[cs][cs][IM][xx] +=
                                            twisted_mass_mu;
                                    }
                                }
                            }
        ...
        for (auto isign : {1, -1}) {
            for (auto target_cb : {0, 1}) {
                ...
                const FullClover *two_flav_invclov[2][2][2] = {
                    {{invclov_packed[target_cb][0],
                      invclov_packed[target_cb][1]},
        ...
        for (auto isign : {1, -1}) {
            for (auto target_cb : {0, 1}) {
                ...
                tmdslash.dslashAChiMinusBDPsi(
                    spinor_result_base_up[target_cb],
                ...
                tmdslash.dslashAChiMinusBDPsi(
                    spinor_result_base_down[target_cb],
