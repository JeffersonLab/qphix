...
void printHelp() 
{ 
       cout << "t_minvcg -x Lx -y Ly -z Lz -t Lt -i iters -by BY -bz BZ -c NCores  -sy SY -sz SZ  -pxy Pxy -pxyz Pxyz -minct MinCt -compress12 -prec Prec" << endl;
...
int main(int argc, char **argv)
{
  ...
  for(int i=0; i < iters; i++) { 
    tests.addTest(new MInvCGTester(By_user, Bz_user, NCores_user, Sy_user, Sz_user, PadXY_user, PadXYZ_user, MinCt_user, compress12, prec_user, QPHIX_SOALEN), "MinvCGTester\n" );
  }
