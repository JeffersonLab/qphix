                              ...
                              const uint_fast64_t max_errors = 0)
{
  ...
  Double diff_norm =
      sqrt(norm2(diff, rb[target_cb])) / (Real(4 * 3 * 2 * Layout::vol()) / Real(2));
    ...
    for (int z = 0; z < Nz; z++) {
      for (int y = 0; y < Ny; y++) {
        for (int x = 0; x < Nxh; x++) {
          ...
          for (int s = 0; s < Ns; s++) {
            for (int c = 0; c < Nc; c++) {
              auto d = diff.elem(rb[target_cb].start() + ind).elem(s).elem(c);
              auto c1 = chi1.elem(rb[target_cb].start() + ind).elem(s).elem(c);
              auto c2 = chi2.elem(rb[target_cb].start() + ind).elem(s).elem(c);
