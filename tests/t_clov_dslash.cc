...
void printHelp() 
{ 
       cout << "t_dslash Test Specific Args: -x Lx -y Ly -z Lz -t Lt -i iters -prec Prec" << endl;
...
int main(int argc, char **argv)
{
  ...
  omp_set_num_threads(some_user_args.getNCores()
		  	  *some_user_args.getSy()* some_user_args.getSz());

  TestRunner  tests(&argc, &argv, nrow_in);

  const multi1d<int>& localLattSize = Layout::subgridLattSize();

  tests.addTest(new testClovDslashFull(some_user_args,compress12,prec_user), "testClovDslashFull" );
