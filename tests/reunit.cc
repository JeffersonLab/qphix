            ...
            enum Reunitarize ruflag)
{
    ...
    RealF fuzz = 1.0e-5; // some kind of small floating point number, should be
                         // prec. dep.
    ...
    // SU(2)
    case 2:
        /* If you want to check unitarity, I have to save the second row
         * somewhere */
        if (ruflag == REUNITARIZE_ERROR || ruflag == REUNITARIZE_LABEL) {
            for (int c = 0; c < Nc; ++c)
                row[c] = a[c][Nc - 1];
        ...
        if (Nc > 3) {
            ...
            case REUNITARIZE_ERROR:
                ...
                numbad = toInt(sum(where(
                    sigmasq > fuzz, LatticeInteger(1), LatticeInteger(0))));
                if (numbad > 0)
                    QDP_error_exit("Unitarity violated", numbad);
                break;
            case REUNITARIZE_LABEL:
                ...
                bad = sigmasq > fuzz;
                numbad = toInt(
                    sum(where(bad, LatticeInteger(1), LatticeInteger(0))));
            default:
                break;
            }
        } else
