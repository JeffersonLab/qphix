...
public:
    ...
    void packForQUDA(multi1d<QUDAPackedClovSite<REALT>> &quda_pack,
                     int cb) const;
...
private:
    ...
    multi1d<bool>
        choles_done; // Keep note of whether the decomposition has been done
                                   ...
                                   const LLVMCloverTermT<T, U> &from)
{
    ...
    {
        RealT ff = where(
            param.anisoParam.anisoP, Real(1) / param.anisoParam.xi_0, Real(1));
                                   ...
                                   const CloverFermActParams &param_)
{
    ...
    QDPIO::cout << "Param Mass=" << param.Mass << " diag_mass = " << diag_mass
                << std::endl;
    /* Calculate F(mu,nu) */
    multi1d<U> f;
    mesField(f, u);
...
void LLVMCloverTermT<T, U>::printDiag(void) const
{
    ...
    for (int site = 0; site < 8; ++site) {
        ...
        for (int block = 0; block < 2; ++block) {
            ...
            for (int d = 0; d < 6; ++d) {
                QDPIO::cout
                    << tri_dia.elem(site).comp[block].diag[d].elem().elem()
                    << " ";
            }
            QDPIO::cout << " )  ";
        }
                              ...
                              const Y &tri_off)
{
    ...
    typename UJIT::Subtype_t &f0_j =
        f0_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename UJIT::Subtype_t &f1_j =
        f1_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename UJIT::Subtype_t &f2_j =
        f2_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename UJIT::Subtype_t &f3_j =
        f3_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename UJIT::Subtype_t &f4_j =
        f4_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename UJIT::Subtype_t &f5_j =
        f5_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);

    typename XJIT::Subtype_t &tri_dia_j =
        tri_dia_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename YJIT::Subtype_t &tri_off_j =
        tri_off_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    ...
    for (int i = 1; i < Nc; ++i) {
        for (int j = 0; j < i; ++j) {
            ...
            tri_off_j.elem(0).elem(elem_tmp) -=
                tri_off_j.elem(0).elem(elem_ij); // * -1.0;
...
template <typename T, typename U>
void LLVMCloverTermT<T, U>::makeClov(const multi1d<U> &f,
                                     const RealT &diag_mass)
{
    ...
    if (!function.built()) {
        ...
        function_make_clov_build(
            function, diag_mass, f0, f1, f2, f3, f4, f5, tri_dia, tri_off);
    }

    // Execute the function
    function_make_clov_exec(
        function, diag_mass, f0, f1, f2, f3, f4, f5, tri_dia, tri_off);
}
...
Double LLVMCloverTermT<T, U>::cholesDet(int cb) const
{
    ...
    if (choles_done[cb] == false) {
        QDPIO::cout << __func__ << ": Error: you have not done the Cholesky.on "
                                   "this operator on this subset"
                    << std::endl;
        QDPIO::cout << "You sure you should not be asking invclov?"
                    << std::endl;
        QDP_abort(1);
    }
...
{
    if (!s.hasOrderedRep())
        QDP_error_exit("ldagdlinv on subset with unordered representation not "
                       "implemented");
                              ...
                              const Subset &s)
{
    ...
    typename TJIT::Subtype_t &tr_log_diag_j =
        tr_log_diag_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename XJIT::Subtype_t &tri_dia_j =
        tri_dia_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename YJIT::Subtype_t &tri_off_j =
        tri_off_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
...
void LLVMCloverTermT<T, U>::ldagdlinv(LatticeREAL &tr_log_diag, int cb)
{
    ...
    if (!function.built()) {
        ...
        function_ldagdlinv_build<U>(
            function, tr_log_diag, tri_dia, tri_off, rb[cb]);
    }
 ...
 *              Adapted from Golub and Van Loan, Matrix Computations, 2nd, Sec
 * 4.2.4
 *
 * Arguments:
 *
                             ...
                             const Subset &s)
{
    ...
    typename UJIT::Subtype_t &B_j =
        B_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename XJIT::Subtype_t &tri_dia_j =
        tri_dia_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    typename YJIT::Subtype_t &tri_off_j =
        tri_off_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
...
void LLVMCloverTermT<T, U>::triacntr(U &B, int mat, int cb) const
{
    ...
    if (mat < 0 || mat > 15) {
        QDPIO::cerr << __func__ << ": Gamma out of range: mat = " << mat
                    << std::endl;
        QDP_abort(1);
    }
...
template <typename T, typename U>
Real LLVMCloverTermT<T, U>::getCloverCoeff(int mu, int nu) const
{
                               ...
                               const Subset &s)
{
    ...
    typename TJIT::Subtype_t &chi_j =
        chi_jit.elem(JitDeviceLayout::LayoutCoalesced, idx);
    ...
    // RComplex<REALT>* cchi =
    // (RComplex<REALT>*)&(chi.elem(site).elem(0).elem(0));
    // const RComplex<REALT>* ppsi = (const
    // RComplex<REALT>*)&(psi.elem(site).elem(0).elem(0));

    int n = 2 * Nc;

    for (int i = 0; i < n; ++i) {
        chi_r.elem((0 * n + i) / 3).elem((0 * n + i) % 3) =
            tri_dia_r.elem(0).elem(i) *
            psi_r.elem((0 * n + i) / 3).elem((0 * n + i) % 3);
        // cchi[0*n+i] = tri[site].diag[0][i] * ppsi[0*n+i];

        chi_r.elem((1 * n + i) / 3).elem((1 * n + i) % 3) =
            tri_dia_r.elem(1).elem(i) *
            psi_r.elem((1 * n + i) / 3).elem((1 * n + i) % 3);
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < i; j++) {
            chi_r.elem((0 * n + i) / 3).elem((0 * n + i) % 3) +=
                tri_off_r.elem(0).elem(kij) *
                psi_r.elem((0 * n + j) / 3).elem((0 * n + j) % 3);
            // cchi[0*n+i] += tri[site].offd[0][kij] * ppsi[0*n+j];

            chi_r.elem((0 * n + j) / 3).elem((0 * n + j) % 3) +=
                conj(tri_off_r.elem(0).elem(kij)) *
                psi_r.elem((0 * n + i) / 3).elem((0 * n + i) % 3);
            // cchi[0*n+j] += conj(tri[site].offd[0][kij]) * ppsi[0*n+i];

            chi_r.elem((1 * n + i) / 3).elem((1 * n + i) % 3) +=
                tri_off_r.elem(1).elem(kij) *
                psi_r.elem((1 * n + j) / 3).elem((1 * n + j) % 3);
            // cchi[1*n+i] += tri[site].offd[1][kij] * ppsi[1*n+j];

            chi_r.elem((1 * n + j) / 3).elem((1 * n + j) % 3) +=
                conj(tri_off_r.elem(1).elem(kij)) *
                psi_r.elem((1 * n + i) / 3).elem((1 * n + i) % 3);
 ...
 */
template <typename T, typename U>
void LLVMCloverTermT<T, U>::apply(T &chi, const T &psi, int isign, int cb) const
{

    if (Ns != 4) {
        QDPIO::cerr << __func__ << ": CloverTerm::apply requires Ns==4"
                    << std::endl;
        QDP_abort(1);
    }
...
void qudaPackSiteLoop(int lo, int hi, int myId, QUDAPackArgs<R, TD, TO> *a)
{
    ...
    for (int ssite = lo; ssite < hi; ++ssite) {
        ...
        // First Chiral Block
        for (int i = 0; i < 6; i++) {
            quda_array[site].diag1[i] =
                tri_dia.elem(site).comp[0].diag[i].elem().elem();
        }
        ...
        for (int col = 0; col < Nc * Ns2 - 1; col++) {
            for (int row = col + 1; row < Nc * Ns2; row++) {
                ...
                quda_array[site].offDiag1[target_index][0] =
                    tri_off.elem(site).comp[0].offd[source_index].real().elem();
                quda_array[site].offDiag1[target_index][1] =
                    tri_off.elem(site).comp[0].offd[source_index].imag().elem();
                target_index++;
            }
        }
        // Second Chiral Block
        for (int i = 0; i < 6; i++) {
            quda_array[site].diag2[i] =
                tri_dia.elem(site).comp[1].diag[i].elem().elem();
        }
        ...
        for (int col = 0; col < Nc * Ns2 - 1; col++) {
            for (int row = col + 1; row < Nc * Ns2; row++) {
                ...
                quda_array[site].offDiag2[target_index][0] =
                    tri_off.elem(site).comp[1].offd[source_index].real().elem();
                quda_array[site].offDiag2[target_index][1] =
                    tri_off.elem(site).comp[1].offd[source_index].imag().elem();
                target_index++;
            }
        }
    }
...
template <typename T, typename U>
void LLVMCloverTermT<T, U>::packForQUDA(
    multi1d<QUDAPackedClovSite<typename WordType<T>::Type_t>> &quda_array,
    int cb) const
{
    typedef typename WordType<T>::Type_t REALT;
    ...
    QDPCloverEnv::QUDAPackArgs<REALT, TD, TO> args = {
        cb, quda_array, tri_dia, tri_off};
    dispatch_to_threads(
        num_sites, args, QDPCloverEnv::qudaPackSiteLoop<REALT, TD, TO>);
...
template <typename T, typename U>
void LLVMCloverTermT<T, U>::applySite(T &chi,
                                      const T &psi,
                                      int isign,
                                      int site) const
{
    QDP_error_exit("LLVMCloverTermT<T,U>::applySite(T& chi, const T& psi,..) "
                   "not implemented ");
#if 0
#endif
}
