...
namespace QPhiX {
  ...
  template <typename FT, int veclen, int soalen, bool compress>
    void TMClovDslash<FT,veclen,soalen,compress>::packFaceDir(int tid,
  ...
  template<typename FT, int veclen,int soalen, bool compress>
    void TMClovDslash<FT,veclen,soalen,compress>::completeFaceDir(int tid,
        const FT* psi,
        FourSpinorBlock* res,
        ...
        int cb, int dir, int fb, int isPlus)
    {
      ...
      for(int pkt = low_pkt + smtid; pkt < high_pkt; pkt+=n_threads_per_core) {
        ...
        const FullCloverBlock *clBase = &invclov[(t*Pxyz+z*Pxy+yi*n_soa_x)/ngy+xblock];
        ...
        if(isPlus)
          tm_clov_face_finish_dir_plus<FT,veclen,soalen,compress>(inbuf, gBase, oBase, clBase, gOffs, offs, hsprefdist, gprefdist, soprefdist, clprefdist, beta_T, mask, dir*2+fb);
        else
          tm_clov_face_finish_dir_minus<FT,veclen,soalen,compress>(inbuf, gBase, oBase, clBase, gOffs, offs, hsprefdist, gprefdist, soprefdist, clprefdist, beta_T, mask, dir*2+fb);
  ...
  template<typename FT, int veclen,int soalen, bool compress>
    void TMClovDslash<FT,veclen,soalen,compress>::completeFaceDirAChiMBDPsi(int tid,
        const FT* psi,
        FourSpinorBlock* res,
        const SU3MatrixBlock* u,
        const double beta,
