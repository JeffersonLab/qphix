...
void TMClovDslash<FT, veclen, soalen, compress>::packFaceDir(
    int tid,
...
template <typename FT, int veclen, int soalen, bool compress>
void TMClovDslash<FT, veclen, soalen, compress>::completeFaceDir(
    int tid,
    const FT *psi,
    FourSpinorBlock *res,
    ...
    bool const is_plus)
{
  ...
  for (int pkt = low_pkt + smtid; pkt < high_pkt; pkt += n_threads_per_core) {
    ...
    const int clov_line_in_floats =
        sizeof(FullCloverBlock) / sizeof(FT); // One gauge scanline, in floats
...
template <typename FT, int veclen, int soalen, bool compress>
void TMClovDslash<FT, veclen, soalen, compress>::completeFaceDirAChiMBDPsi(
    int tid,
    const FT *psi,
    FourSpinorBlock *res,
    const SU3MatrixBlock *u,
