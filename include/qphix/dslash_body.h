...
#pragma omp parallel 
  ...
  template<typename FT, int veclen, int soalen, bool compress12>
    void Dslash<FT,veclen,soalen,compress12>::DPsi(const SU3MatrixBlock *u, const FourSpinorBlock *psi_in, FourSpinorBlock *res_out, int cb)
      {
...
#pragma omp barrier
				  ...
				  if ( !cantDoPFaces ) {
					  ...
					  // d == 0 && s->Nxh == soalen
					  completeFaceDir2(tid,tid, nthread,comms->recvFromDir[2*d+0], res_out, u, beta_s,cb, d, 0, 1);
					  completeFaceDir2(tid,tid, nthread,comms->recvFromDir[2*d+1], res_out, u, beta_s, cb, d, 1, 1);
				  }
			  } // if
		  } // for
	  }//scope
  ...
  template<typename FT, int veclen, int soalen, bool compress12>
    void Dslash<FT,veclen,soalen, compress12>::DPsiAChiMinusBDPsi(const SU3MatrixBlock *u, 
