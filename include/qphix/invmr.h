...
class InvMRBase
{
 public:
	   ...
	   bool terminateOnResiduum) const
  {
    ...
    if(MaxIters < 0) {
    	masterPrintf("ERROR: MR invalid value: MaxIters=%d\n", MaxIters);
    }
    if( MaxIters == 0 ) {
    	//No work to do
    	n_iters=0;
	...
	rsd_sq *= norm_chi;
      ...
      if( verbose ) {
	masterPrintf("MR: iter=0: || r ||^2 = %16.8e Target || r ||^2 = %16.8e\n",cp, rsd_sq);
      }
      if( cp < rsd_sq ) {
	n_iters = 0;
	rsd_sq_final = cp;

	if( residType == ABSOLUTE  && verbose) {
	  masterPrintf("MR: Final iters=0 || r ||_accum=16.8e || r ||_actual = %16.8e\n",
		       sqrt(rsd_sq_final), sqrt(rsd_sq_final));
	}
	else {
	  rsd_sq_final /= norm_chi;
	  if( verbose ) {
	    masterPrintf("MR: iters=0 || r ||/|| b ||_accum=16.8e || r ||/|| b ||_actual = %16.8e\n",
			 sqrt(rsd_sq_final), sqrt(rsd_sq_final));
	...
	if( terminateOnResiduum ) {
	  ...
	  if( verbose ) {
	    masterPrintf("MR: iter=%d: || r ||^2 = %16.8e Target || r ||^2 = %16.8e\n",k, cp, rsd_sq);
	  } // verbose
	  continueP = ( k < MaxIters ) && ( cp > rsd_sq ) ;
	}
	else {
	  if( verbose) {
	    masterPrintf("MR: iter=%d\n", k);
	  }
    ...
    rsd_sq_final = cp;
    if( residType == ABSOLUTE  && verbose) {
      masterPrintf("MR: Final iters=%d || r ||_accum=%16.8e || r ||_actual = %16.8e\n", k,
		   sqrt(rsd_sq_final), sqrt(rsd_sq_final));
    }
    else {
      rsd_sq_final /= norm_chi;
      if( verbose ) {
	masterPrintf("MR: iters=%d || r ||/|| b ||_accum=%16.8e || r ||/|| b ||_actual = %16.8e\n",k,
		     sqrt(rsd_sq_final), sqrt(rsd_sq_final));
