...
class TwoFlavTwistedMassFunctor
{
 public:
  ...
  inline void func(int block)
  {
    BLASUtils::StreamInSpinor<FT, AT, V, S, compress> x_up(
        &x[0][block][0][0][0][0]);
    BLASUtils::StreamInSpinor<FT, AT, V, S, compress> x_dn(
        &x[1][block][0][0][0][0]);

    BLASUtils::StreamSpinor<FT, AT, V, S, compress> y_up(
        false, BLASUtils::StreamOut::stream, &y[0][block][0][0][0][0]);
    BLASUtils::StreamSpinor<FT, AT, V, S, compress> y_dn(
        false, BLASUtils::StreamOut::stream, &y[1][block][0][0][0][0]);

    for (int col = 0; col < 3; col++) {
      for (int spin = 0; spin < 4; spin++) {
        // (a + i mu gamma_5 tau_3 + epsilon tau_1) \psi
        (spin < 2 ? BLASUtils::tau3cm_tau1_scaleadd(y_up.get()[col][spin],
                                                    y_dn.get()[col][spin],
                                                    apimu,
                                                    epsilon,
                                                    x_up.get()[col][spin],
                                                    x_dn.get()[col][spin])
                  : BLASUtils::tau3cconjm_tau1_scaleadd(y_up.get()[col][spin],
                                                        y_dn.get()[col][spin],
                                                        apimu,
                                                        epsilon,
                                                        x_up.get()[col][spin],
