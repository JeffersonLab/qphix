...
#else
	outbuf << "__m512i " << vname << " = _mm512_load_epi32(" << pname << ", _MM_UPCONV_EPI32_NONE, _MM_BROADCAST32_NONE, _MM_HINT_NONE);" << endl;
#endif
	return outbuf.str();
}
...
#else
	buf << v.getName() << " = _mm512_mask_load_ps(" << v.getName() << ", " << lmask << ", "  << a->serialize() << ", " << upConv << " _MM_BROADCAST32_NONE, _MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
...
#else
	buf << "_mm512_store_ps(" << a->serialize() << "," << v.getName() <<  ", " << downConv <<  ",_MM_HINT_NONE);" <<endl;
#endif
	return buf.str();
}
...
public:
	...
	string serialize() const {
		...
		stream << ret.getName() << " = _mm512_mask_permute4f128_ps(" << ret.getName() << ", " << mk << ", " << a.getName() << ", (_MM_PERM_ENUM)" << imm << ");" ;
		return stream.str();
	}
